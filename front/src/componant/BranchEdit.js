...
const BranchEdit = () => {
  ...
  const getCityData = async (s_id, c_id) => {
    try {
      const res = await axios.get(
        `${process.env.REACT_APP_BACKEND_BASE_URL}/getCitiesByCountryAndState/?s_id=${s_id}&c_id=${c_id}`
      );
      if (res.status === 200) {
  ...
  const getAreaData = async (city_id, s_id, c_id) => {
    try {
      const res = await axios.get(
        `${process.env.REACT_APP_BACKEND_BASE_URL}/getareabycountryandstateandcity/?c_id=${c_id}&s_id=${s_id}&city_id=${city_id}`
      );
      if (res.status === 200) {
  ...
  const getBranchData = async () => {
    try {
      const res = await axios.get(`${process.env.REACT_APP_BACKEND_BASE_URL}/branch/${id}`);
      if (res.status === 200 && res.data.data.length > 0) {
        setBranch({
          name: res.data.data[0].name,
          email: res.data.data[0].email,
          contact: res.data.data[0].contact,
          id_country: res.data.data[0].id_country,
          id_state: res.data.data[0].id_state,
          id_city: res.data.data[0].id_city,
          id_area: res.data.data[0].id_area
        });
        getStateData(res.data.data[0].id_country);
        getCityData(res.data.data[0].id_state, res.data.data[0].id_country)
        getAreaData(res.data.data[0].id_city, res.data.data[0].id_state, res.data.data[0].id_country)
      } else {
        Swal.fire({
          type: 'error',
          icon: 'error',
  ...
  const handleUpdate = async (event) => {
    ...
    try {
      ...
      if (!trimmedBranch.name || !trimmedBranch.email || !trimmedBranch.contact || !branch.name || !branch.email || !branch.contact || !branch.id_country || !branch.id_state || !branch.id_city || !branch.id_area) {
        Swal.fire({
          title: 'Please enter all Data ',
          icon: 'warning',
        });
      } else if (!emailRegex.test(branch.email)) {
      ...
      } else if (!contactRegex.test(branch.contact)) {
        Swal.fire({
        ...
        const res = await axios.put(
          `${process.env.REACT_APP_BACKEND_BASE_URL}/branch/update/${id}`, branch);
        if (res.status === 200) {
          Swal.fire({
            icon: 'success',
              ...
              <div className="row">
                <nav aria-label="breadcrumb" style={{ '--bs-breadcrumb-divider': 'none' }}>
                  <ol className="breadcrumb">
                    ...
                    <li className="breadcrumb-item"><NavLink to="/branch">Branch </NavLink>/</li>
                    <li className="breadcrumb-item active" aria-current="page">Edit Branch</li>
                  </ol>
                </nav>
                  ...
                  <div className="row">
                    <div className="col-lg-6">
                      ...
                      <div className="form-group">
                        ...
                        <div>
                          <select
                            ...
                            value={branch.id_country}
                            onChange={(e) => {
                              setSelectedCountry(e.target.value);
                              getStateData(e.target.value, branch.id_country);
                              handleInputChange(e);
                            }}
                          >
                      ...
                      <div className="form-group">
                        ...
                        <div>
                          <select
                            ...
                            value={branch.id_state}
                            onChange={(e) => {
                              setSelectedState(e.target.value);
                              getCityData(e.target.value, branch.id_country, branch.id_state);
                              handleInputChange(e);
                            }}
                          >
                            <option selected >
                      ...
                      <div className="form-group">
                        ...
                        <div>
                          <select
                            ...
                            value={branch.id_city}
                            onChange={(e) => {
                              setSelectedCity(e.target.value);
                              getAreaData(e.target.value, branch.id_country, branch.id_state, branch.id_city);
                              handleInputChange(e);
                            }}
                          >
                            <option selected>
                      ...
                      <div className="btn-path">
                        <Link to={'/branch'} className="btn btn-cancel me-3">
                          Back
                        </Link>
