...
function CreateStandard() {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      if (!trimmedStandard.standard || !standard.standard || !standard.branch_id || !standard.id_board || !standard.id_medium || standard.id_board === "--Select Board--" || standard.id_medium === "--Select Medium--") {
        Swal.fire({
        ...
        const res = await axios.post(
          `${process.env.REACT_APP_BACKEND_BASE_URL}/create-standerd`,
          standard
        );
        ...
        if (res.status === 200) {
          ...
          Swal.fire({
            icon: 'success',
            title: 'Standard Inserted Successfully',
            timer: 1500,
          }).then(() => {
            navigate('/standard');
          });
          ...
          Swal.fire({
            title: 'Standard insertion failed',
            icon: 'error',
          });
        }
      }
              ...
              <div className="row">
                <nav aria-label="breadcrumb" style={{ '--bs-breadcrumb-divider': 'none' }}>
                  <ol className="breadcrumb">
                    ...
                    <li className="breadcrumb-item active" aria-current="page">Add Standard</li>
                  </ol>
                </nav>
                  ...
                  <div className="row">
                    <div className="col-lg-6">
                      ...
                      <div className='from-group mt-4'>
                        ...
                        >
                          <option selected value="--Select Medium--">
                            Select Medium
                          </option>
                          {medium.length > 0 &&
                            medium.map((data) => (
