  ...
  const getMaterialData = async () => {
    try {
      const res = await axios.get(
        `${process.env.REACT_APP_BACKEND_BASE_URL}/getmaterial`
      );
      if (res.status === 200) {
        if (userData.roll === "Admin") {
  ...
  const handleDelete = async (id) => {
    try {
      const result = await Swal.fire({
        title: "Are you sure?",
        text: "You will be delete this material",
        icon: "warning",
      ...
      if (result.isConfirmed) {
        const res = await axios.delete(
          `${process.env.REACT_APP_BACKEND_BASE_URL}/material/delete/` + id
        );
        if (res.status === 200) {
          getMaterialData();
          Swal.fire({
          ...
          <div className="content">
            <nav aria-label="breadcrumb" style={{ '--bs-breadcrumb-divider': 'none' }}>
              <ol className="breadcrumb">
                ...
                <li className="breadcrumb-item active" aria-current="page">Material</li>
              </ol>
            </nav>
                        ...
                        currentItems.map((dataa, index) => {
                          return (
                            <tr key={index}>
                              ...
                              {/* <td>
                              {Array.isArray(dataa.files)
                                ? dataa.files.map((file) => (
                                  <div key={file.name}>
                                    {file.name.replace(/"/g, "")}
                                  </div>
                                ))
                                : dataa.files
                                  ? dataa.files
                                    ...
                                    .map((file) => (
                                      <div key={file}>
                                        <Link to={`${process.env.REACT_APP_BACKEND_BASE_URL}/uploads/${file.replace(/"/g, "").trim()}`} target="_blank">
                              ...
                              <td>
                                <Link to={`${process.env.REACT_APP_BACKEND_BASE_URL}/uploads/${dataa.files}`} target="_blank">
                                  <div className="delete-table"                                          >
                                    <i class="fa-solid fa-file"></i>
                                  </div>
                                </Link>
