...
export default function Board() {
    ...
    try {
      ...
      if (result.isConfirmed) {
        const res = await axios.delete(
          `${process.env.REACT_APP_BACKEND_BASE_URL}/delete-board/${id}`
        );
        if (res.status === 200) {
  ...
  const filterDataBySearch = (data, query) => {
    return data.filter((item) =>
      item.board.toLowerCase().includes(query.toLowerCase())
    );
  };
  ...
  const handleSearch = (e) => {
    ...
    const filteredData = filterDataBySearch(data, query);
    const filteredFilteredData = filterDataBySearch(filteredData, query);
    setFilteredData(filteredFilteredData);
  };
          ...
          <div className="content">
            ...
            <div className="d-flex justify-content-between">
              ...
              <div className="list-btn mt-4">
                <ul>
                  <li>
                    <Link to={"/createboard"} className="btn btn-primary">
