...
const Branch = () => {
    ...
    try {
      ...
      if (result.isConfirmed) {
        const res = await axios.delete(
          `${process.env.REACT_APP_BACKEND_BASE_URL}/branch/delete/` + id
        );
        if (res.status === 200) {
          getBranchData()
          Swal.fire({
            title: "Deleted!",
            text: "This branch has been deleted.",
            icon: "success",
  ...
  const getAreaNameById = (areaId) => {
    const foundArea = area.find((area) => area.id === areaId);
  ...
  useEffect(() => {
    setFilteredData(filterDataBySearch(branch ?? []));
  }, [branch, searchQuery]);
          ...
          <div className="content">
            ...
            <div className="d-flex justify-content-between">
              ...
              <div className="list-btn mt-4">
                <ul>
                  <li>
                    <Link to={"/addbranch"} className="btn btn-primary">
                        ...
                        filteredData.map((branch, index) => {
                          return (
                            <tr key={index}>
                              <td>{index + 1}</td>
                              <td>{branch.name}</td>
                              <td className="mail-text">{branch.email}</td>
                              <td>{branch.contact}</td>
                              <td>{getCountryNameById(branch.id_country)}</td>
                              <td>{getStateNameById(branch.id_state)}</td>
                              <td>{getCityNameById(branch.id_city)}</td>
                              <td>{getAreaNameById(branch.id_area)}</td>
                              <td>
                                <div className="table-actions d-flex">
                                  ...
                                  <NavLink
                                    ...
                                    onClick={() => handleDelete(branch.id)} // Pass a function reference
                                  >
                                    <img
