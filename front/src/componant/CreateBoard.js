...
export default function CreateBoard() {
    ...
    const headleSubmit = async (event) => {
        ...
        try {
            ...
            if (!trimmedBoard.board || !values || !values.board || !values.id_branch) {
                Swal.fire({
                    title: "Please enter all data",
                    icon: "warning",
                });
            } else {
                const res = await axios.post(`${process.env.REACT_APP_BACKEND_BASE_URL}/create-board`, values);
                if (res.status === 200) {
                    Swal.fire({
                        icon: 'success',
                        title: 'Board Inserted Successfully',
                        timer: 1500,
                    }).then(() => {
                        navigate('/board');
                    });
                            ...
                            <div className="row">
                                <nav aria-label="breadcrumb" style={{ '--bs-breadcrumb-divider': 'none' }}>
                                    <ol className="breadcrumb">
                                        ...
                                        <li className="breadcrumb-item active" aria-current="page">Add Board</li>
                                    </ol>
                                </nav>
                                <div className="col-lg-12 col-sm-12">
                                    ...
                                    <div className="col-lg-6 mt-3">
                                        <div className="form-group">
                                            <label>Board</label>
                                            <input type="text" className="form-control" placeholder="Enter Board"
