...
export default function EditMedium() {
    ...
    useEffect(() => {
        getBranchData();
        axios.get(`${process.env.REACT_APP_BACKEND_BASE_URL}/getedit-medium/` + id)
            .then(res => {
                console.log(res);
                setValues({ ...values, medium: res.data.data[0].medium, id_branch: res.data.data[0].id_branch, id_board: res.data.data[0].id_board })
    ...
    const headleUpdate = async (event) => {
        ...
        try {
            ...
            if (!trimmedMedium.medium || !values.medium || !values.id_branch || !values.id_board || values.id_board === "-- Select Board --") {
                Swal.fire({
                ...
                const res = await axios.put(
                    `${process.env.REACT_APP_BACKEND_BASE_URL}/edit-medium/${id}`, values);
                if (res.status === 200) {
                    Swal.fire({
                        icon: 'success',
                        title: 'Medium Updated Successfully',
                        timer: 1500,
                    }).then(() => {
                        navigate('/medium');
                    });
                            ...
                            <div className="row">
                                <nav aria-label="breadcrumb" style={{ '--bs-breadcrumb-divider': 'none' }}>
                                    <ol className="breadcrumb">
                                        ...
                                        <li className="breadcrumb-item"><NavLink to="/medium">Medium </NavLink>/</li>
                                        <li className="breadcrumb-item active" aria-current="page">Edit Medium</li>
                                    </ol>
                                </nav>
                                    ...
                                    <div className="col-6 mt-3">
                                        <div className="form-group">
                                            ...
                                            <div className="btn-path">
                                                <Link to={'/medium'} className="btn btn-cancel me-3" >Back</Link>
