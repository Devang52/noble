...
function EditExpanse() {
    ...
    const getExpenseTypeData = async () => {
        try {
            const res = await axios.get(`${process.env.REACT_APP_BACKEND_BASE_URL}/expanse-type`);
            if (res.status === 200) {
    ...
    const getExpenseData = async () => {
        try {
            const res = await axios.get(`${process.env.REACT_APP_BACKEND_BASE_URL}/getexpanse/${id}`);
            if (res.status === 200 && res.data.data.length > 0) {
                setExpense({
                    name: res.data.data[0].name,
                    id_branch: res.data.data[0].id_branch,
                    expense_name: res.data.data[0].expense_name,
                    expense_type: res.data.data[0].expense_type,
    ...
    const handleUpdate = async (event) => {
        ...
        try {
            const trimmedExpanse = {
                name: expense.expense_name.trim(),
            };
            if (!trimmedExpanse.name || !expense.id_branch || !expense.expense_name || !expense.expense_type || !expense.amount) {
                Swal.fire({
                ...
                const res = await axios.put(
                    `${process.env.REACT_APP_BACKEND_BASE_URL}/expanse/update/${id}`, expense);
                if (res.status === 200) {
                    Swal.fire({
                        icon: 'success',
                    ...
                    <div className="content">
                        <nav aria-label="breadcrumb" style={{ '--bs-breadcrumb-divider': 'none' }}>
                            <ol className="breadcrumb">
                                ...
                                <li className="breadcrumb-item"><NavLink to="/expanse">Expense </NavLink>/</li>
                            ...
                            <div className="row">
                                ...
                                <div className="col-6">
                                    ...
                                    <div className="btn-path">
                                        <NavLink to="/expanse" className="btn btn-cancel me-3">
                                            Back
                                        </NavLink>
