...
function Subject() {
    ...
    try {
      ...
      if (res.status === 200) {
        if (userData.roll === 'Admin') {
          ...
          setSubject(res.data.data.filter(board => board.id_branch === branchId));
        }
      } else {
        setSubject([]);
      }
  ...
  const handleDelete = async (id) => {
    try {
      const result = await Swal.fire({
        title: "Are you sure?",
        text: "You will be delete this Subject",
        icon: "warning",
      ...
      if (result.isConfirmed) {
        const res = await axios.delete(
          `${process.env.REACT_APP_BACKEND_BASE_URL}/subject/delete/` + id
        );
        if (res.status === 200) {
          getSubjectData();
          Swal.fire({
  ...
  useEffect(() => {
    setFilteredData(filterDataBySearch(subject));
  }, [subject, searchQuery]);
          ...
          <div class="content">
            ...
            <div class="content-page-header content-page-headersplit">
              ...
              <div class="list-btn mt-4">
                <ul>
                  <li>
                    <Link to={"/createsubject"} class="btn btn-primary">
                        ...
                        currentItems.map((sub, index) => {
                          return (
                            <tr key={index}>
                              <td>{index + 1}</td>
                              <td>
                                {sub.subject.charAt(0).toUpperCase() +
                                  sub.subject.slice(1)}
                              </td>
                              <td>{sub.branch_name}</td>
                              <td>{sub.board_name}</td>
                              <td>{sub.medium_name}</td>
                              <td>{sub.standard_name}</td>
                              <td>
                                <div className="table-actions d-flex">
                                  <Link
                                    className="delete-table me-2"
                                    to={`/editsubject/${sub.id}`}
                                  >
                                    <i className="fas fa-edit"></i>
                                  </Link>
                                  <NavLink
                                    className="delete-table delll"
                                    onClick={() => handleDelete(sub.id)} // Pass a function reference
                                  >
                                    <img
