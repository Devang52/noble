...
function AddCourse() {
  ...
  const getBoardData = async (branchIds) => {
    try {
      const res = await axios.get(
        `${process.env.REACT_APP_BACKEND_BASE_URL}/boardbybr/?id_branch=${branchIds}`
      );
      if (res.status === 200) {
  ...
  const handleInputChange = (e) => {
    ...
    if (name === "id_branch" || name === "id_subject") {
      setCourse({
        ...course,
    ...
    try {
      ...
      if (
        ...
        const res = await axios.post(
          `${process.env.REACT_APP_BACKEND_BASE_URL}/course`,
          course
        );
        ...
        if (res.status === 200) {
          ...
          Swal.fire({
            icon: "success",
            title: "Course insertion successful",
            timer: 1500,
          }).then(() => {
            navigate("/course");
          });
