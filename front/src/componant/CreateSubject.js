  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const trimmedSubject = {
        ...
        id_stand: subject.id_stand.trim(),
        id_branch: subject.id_branch.trim(),
        id_board: subject.id_board.trim(),
        id_medium: subject.id_medium.trim(),
      };
      ...
      if (
        ...
        const res = await axios.post(
          `${process.env.REACT_APP_BACKEND_BASE_URL}/subject`,
          trimmedSubject
        );
        ...
        if (res.status === 200) {
          ...
          Swal.fire({
            icon: 'success',
            title: 'Subject Inserted Successfully',
            timer: 1500,
          }).then(() => {
            navigate('/subject');
          });
          ...
          Swal.fire({
            title: 'Subject insertion failed',
            icon: 'error',
          });
        }
      }
                          ...
                          <select
                            ...
                            onChange={(e) => {
                              ...
                              setSubject((prevValues) => ({
                                ...
                                id_stand: '', // Clear standard on branch change
                              }));
                              setBoard([])
                              setMedium([])
                              setStandard([])
