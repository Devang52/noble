...
function CreateInquiry() {
  ...
  const getDefaultFollowupDate = () => {
    ...
    const formattedDate = defaultFollowupDate.toISOString().split("T")[0];

    return formattedDate;
  };
  const [values, setValues] = useState({
    name: "",
    inquiry_date: getCurrentDate(),
    gender: "",
    id_branch: "",
    school: "",
    id_shift: "",
    ...
    remark: "",
    final_fees: "",
    followup_date: getDefaultFollowupDate(),
    email: "",
  });
  ...
  const handleInputChange = (e) => {
    ...
    } else if (name === "id_reference_type ") {
      setValues({
        ...values,
    ...
    } else if (name === "followup_date") {
      if (values.inquiry_date > value) {
        Swal.fire({
          title: "Followup Date",
          text: "Followup Date cannot be more than Inquiry Date",
          icon: "warning",
        });
      } else {
        setValues({
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      if (!emailValid) {
      ...
      } else if (values.inquiry_date > values.followup_date) {
        Swal.fire({
          title: "Followup Date",
          text: "Followup Date cannot be more than Inquiry Date",
          icon: "warning",
        ...
        const res = await axios.post(
          `${process.env.REACT_APP_BACKEND_BASE_URL}/createinquiry`,
          values
        );
        ...
        if (res.status === 200) {
          Swal.fire({
            icon: "success",
            title: "Inquiry insertion successful",
            timer: 1500,
          }).then(() => {
            navigate("/inquiry");
          });
          ...
          Swal.fire({
            title: "Inquiry insertion failed",
            icon: "error",
          });
        }
      }
              ...
              <div className="row">
                <nav aria-label="breadcrumb" style={{ '--bs-breadcrumb-divider': 'none' }}>
                  <ol className="breadcrumb">
                    ...
                    <li className="breadcrumb-item active" aria-current="page">Add Inquiry</li>
                  </ol>
                </nav>
                    ...
                    <div className="col-lg-6 mt-3">
                      ...
                      <div>
                        <select
                          ...
                          className="form-select"
                          required
                          placeholder="Select Referance Type"
                          onChange={handleInputChange}
                        >
                          <option selected disabled>
