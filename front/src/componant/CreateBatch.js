  ...
  const getMediumData = async (id_branch, id_board) => {
    try {
      ...
      if (res.status === 200) {
        ...
        // Clear the selected standard when medium changes
  ...
  const headleSubmit = async (event) => {
    ...
    try {
      ...
      if (!trimmedBatch.name || !values.name || !values.id_branch || !values.id_shift || !values.id_board || !values.id_medium || !values.id_standard || values.id_board === "-- Select Board--" || values.id_medium === "-- Select Medium--" || values.id_standard === "-- Select Standard--") {
        ...
        const res = await axios.post(`${process.env.REACT_APP_BACKEND_BASE_URL}/createbatch`, values);

        if (res.status === 200) {
          // Successful request
          Swal.fire({
            icon: 'success',
            title: 'Batch insertion successful',
            timer: 1500,
          }).then(() => {
            navigate('/batch');
          });
                ...
                <div className="col-lg-12 col-sm-12">
                  ...
                  <div className="row">
                        ...
                        <div>
                          <select
                            ...
                            onChange={(e) => {
                              ...
                              getStandardData(values.id_branch, values.id_board, values)
                            }}
                          >
                    ...
                    <div className="col-lg-7">
                      <div className="form-group">
                        <label>Batch</label>
                        <input type="text" className="form-control" placeholder="Batch" name="name"
                          onChange={handleInputChange} />
                      </div>
                    </div>
                    <div className="col-lg-6">
                      <div className="btn-path">
                        <Link to={'/batch'} className="btn btn-cancel me-3">Back</Link>
