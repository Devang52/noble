...
const EditBatch = () => {
  ...
  const getShiftData = async () => {
    try {
      const res = await axios.get(`${process.env.REACT_APP_BACKEND_BASE_URL}/shift`);
  ...
  const getBatchData = async () => {
    try {
      ...
      const res = await axios.get(`${process.env.REACT_APP_BACKEND_BASE_URL}/batch/${id}`);
      if (res.status === 200 && res.data.data.length > 0) {
        ...
        await getMediumData(res.data.data[0].id_branch, res.data.data[0].id_board);
        await getStandardData(res.data.data[0].id_branch, res.data.data[0].id_board, res.data.data[0].id_medium);
      } else {
        Swal.fire({
          type: 'error',
          icon: 'error',
  ...
  const headleSubmit = async (event) => {
    ...
    try {
      ...
      if (!trimmedBatch.name || !values.name || !values.id_branch || !values.id_shift || !values.id_board || !values.id_medium || !values.id_standard || values.id_board === "-- Select Board--" || values.id_medium === "-- Select Medium--" || values.id_standard === "-- Select Standard--") {
        Swal.fire({
        ...
        const res = await axios.put(
          `${process.env.REACT_APP_BACKEND_BASE_URL}/batch/update/${id}`,
          values
        );
                ...
                <div className="row">
                  <nav aria-label="breadcrumb" style={{ '--bs-breadcrumb-divider': 'none' }}>
                    <ol className="breadcrumb">
                      ...
                      <li className="breadcrumb-item"><NavLink to="/batch">Batch </NavLink>/</li>
                  ...
                  <div className="col-lg-12 col-sm-12">
                    ...
                    <div className="row">
                        ...
                        <div className='from-group mt-4'>
                          ...
                          <select
                            ...
                            onChange={(e) => {
                              ...
                              // getStandardData(values.id_branch, values.id_board, values.id_medium, value);
                            }}
                          >
                      ...
                      <div className="col-lg-7">
                        <div className="form-group">
                          <label>Batch</label>
                          <input type="text" className="form-control" placeholder="Batch" name="name" value={values.name}
                            onChange={handleInputChange} />
                        </div>
                      </div>
                      <div className="col-lg-6">
                        <div className="btn-path">
                          <NavLink to={"/batch"} className="btn btn-cancel me-3">
                            Back
                          </NavLink>
