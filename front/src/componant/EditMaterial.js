...
const EditMaterial = () => {
  ...
  const getMaterialData = async () => {
    try {
      const res = await axios.get(
        `${process.env.REACT_APP_BACKEND_BASE_URL}/getmaterial/${id}`
      );
      if (res.status === 200) {
        console.log(res.data.data);
        setMaterial({
  ...
  const handleUpdate = async (event) => {
    ...
    try {
      ...
      if (
        !trimmedMaterial.name ||
        !material.name ||
        !material.id_branch ||
        !material.id_board ||
        !material.id_medium ||
        !material.id_standard ||
        !material.id_subject ||
        material.id_board === "-- Select Board--" ||
        material.id_medium === "-- Select Medium--" ||
        material.id_standard === "-- Select Standard--" ||
        material.id_subject === "-- Select Subject--"
      ) {
        Swal.fire({
          title: "Please enter material data and select files",
          icon: "warning",
        });
      } else {
        ...
        const res = await axios.put(
          `${process.env.REACT_APP_BACKEND_BASE_URL}/material/update/${id}`, // Replace YOUR_MATERIAL_ID with the actual material id
          formDataObject,
        );
        ...
        if (res.status === 200) {
          ...
          Swal.fire({
            icon: "success",
            title: "Material update successful",
            timer: 1500,
          }).then(() => {
            navigate("/material");
          });
          ...
          <div className="content">
            <nav aria-label="breadcrumb" style={{ '--bs-breadcrumb-divider': 'none' }}>
              <ol className="breadcrumb">
                ...
                <li className="breadcrumb-item"><NavLink to="/material">Material </NavLink>/</li>
                <li className="breadcrumb-item active" aria-current="page">Edit Material</li>
              </ol>
            </nav>
              ...
              <div className="row">
                <div className="col-6">
                  <div className="add-form">
                    <label>Branch</label>
                    <input type="text" className="form-control" disabled value={material.branch_name} />
                    {/* <div>
                    <select
                      name="id_branch"
                      id="id_branch"
                      ...
                      required
                      placeholder="Branch"
                      value={material.id_branch}
                      onChange={(e) => {
                        const { value } = e.target;
                        setMaterial((prevValues) => ({
                          ...prevValues,
                          id_branch: value,
                          id_board: "",
                ...
                <div className="col-6">
                  <div className="add-form">
                    <label>Board</label>
                    <input type="text" className="form-control" disabled value={material.board_name} />
                    {/* <select
                    name="id_board"
                    id="id_board"
                    className="form-select"
                    required
                    value={material.id_board}
                    onChange={(e) => {
                      const { value } = e.target;
                      setMaterial((prevValues) => ({
                        ...prevValues,
                      ...
                      getMediumData(material.id_branch, value);
                    }}
                  >
                ...
                <div className="col-6">
                  <div className="add-form">
                    <label>Medium</label>
                    <input type="text" className="form-control" disabled value={material.medium_name} />
                    {/* <select
                    name="id_medium"
                    id="id_medium"
                    className="form-select"
                    value={material.id_medium}
                    required
                    onChange={(e) => {
                      const { value } = e.target;
                      setMaterial((prevValues) => ({
                      ...
                      getStandardData(material.id_branch, material.id_board, value);
                    }}
                  >
                ...
                <div className="col-6">
                  <div className="add-form">
                    ...
                    <input type="text" className="form-control" disabled value={material.standard_name} />
                    {/* <select
                    name="id_standard"
                    id="id_standard"
                    className="form-select"
                    value={material.id_standard}
                    required
                    onChange={(e) => {
                      const { value } = e.target;
                      setMaterial((prevValues) => ({
                ...
                <div className="col-3 mt-4">
                  ...
                  <input type="text" className="form-control" value={material.files} disabled />
                </div>
                ...
                <div className="btn-path">
                  <Link to={"/material"} className="btn btn-cancel me-3">
                    Back
                  </Link>
