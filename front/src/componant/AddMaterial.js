...
function AddMaterial() {
  ...
  useEffect(() => {
    if (userData) {
      if (userData.roll === "Admin") {
        ...
        setMaterial({ id_branch: branchId })
      }
    }
  }, []);
  ...
  const handleFileChange = (event) => {
    ...
    setMaterial({
      ...material,
      files: files[0], // Only take the first file from the array
    });
  };
  const handleInputChange = (e) => {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      if (
        ...
        !material.id_subject ||
        !material.files || // Check if a file is selected
        ...
        Swal.fire({
          title: "Please enter material data and select a file",
          icon: "warning",
        });
      } else {
        const formData = new FormData();
        formData.append("name", material.name);
        formData.append("id_branch", material.id_branch);
        formData.append("id_board", material.id_board);
        formData.append("id_medium", material.id_medium);
        formData.append("id_standard", material.id_standard);
        formData.append("id_subject", material.id_subject);

        // Append the single file to the form data
        formData.append("files", material.files);

        const res = await axios.post(
          `${process.env.REACT_APP_BACKEND_BASE_URL}/material`,
          formData,
          {
            headers: {
        ...
        if (res.status === 200) {
          ...
          Swal.fire({
            icon: "success",
            title: "Material insertion successful",
            timer: 1500,
          }).then(() => {
            navigate("/material");
          });
          ...
          Swal.fire({
            title: "Material insertion failed",
            icon: "error",
          });
        }
      }
  ...
  //         formData.append("files", material.files[i]);
  //       }
  //       console.log(formData);
          ...
          <div className="content">
            <nav aria-label="breadcrumb" style={{ '--bs-breadcrumb-divider': 'none' }}>
              <ol className="breadcrumb">
                ...
                <li className="breadcrumb-item active" aria-current="page">Add Material</li>
              </ol>
            </nav>
                  ...
                  <div className="form-group">
                    ...
                    <div>
                      <select
                        ...
                        required
                        value={branchSel ? branchSel : material.id_branch}
                        placeholder="Branch"
                        onChange={(e) => {
                          const { value } = e.target;
                          setMaterial((prevMateril) => ({
                            ...prevMateril,
                            id_branch: value,
                            id_board: "",
