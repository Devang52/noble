...
const EditIncome = () => {
  ...
  const getIncomeData = async () => {
    try {
      const res = await axios.get(`${process.env.REACT_APP_BACKEND_BASE_URL}/getincome/${id}`);
      if (res.status === 200 && res.data.data.length > 0) {
        setIncome({
          name: res.data.data[0].name,
          id_branch: res.data.data[0].id_branch,
          income_title: res.data.data[0].income_title,
          income_type: res.data.data[0].income_type,
  ...
  const handleUpdate = async (event) => {
    ...
    try {
      const trimmedIncome = {
        title: income.income_title.trim(),
      };
      if (!trimmedIncome.title || !income.id_branch || !income.income_title || !income.income_type || !income.amount) {
        Swal.fire({
        ...
        const res = await axios.put(
          `${process.env.REACT_APP_BACKEND_BASE_URL}/income/update/${id}`, income);
        if (res.status === 200) {
          Swal.fire({
            icon: 'success',
          ...
          <div className="content">
            <nav aria-label="breadcrumb" style={{ '--bs-breadcrumb-divider': 'none' }}>
              <ol className="breadcrumb">
                ...
                <li className="breadcrumb-item"><NavLink to="/income">Income </NavLink>/</li>
                <li className="breadcrumb-item active" aria-current="page">Edit Income</li>
              </ol>
            </nav>
              ...
              <div className="row">
                ...
                <div className="col-6">
                  ...
                  <div className="btn-path">
                    <NavLink to="/income" className="btn btn-cancel me-3">
                      Back
                    </NavLink>
