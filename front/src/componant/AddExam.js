...
function AddExam() {
  ...
  const headleSubmit = async (event) => {
    ...
    try {
      ...
      if (
      ...
      } else if (!values.id_student.length) {
        Swal.fire({
          title: "Please add students first",
          icon: "warning",
        });
      } else {
        ...
        const res = await axios.post(
          `${process.env.REACT_APP_BACKEND_BASE_URL}/createaddexam`,
          formDataObject
        );
        if (res.status === 200) {
          Swal.fire({
            icon: "success",
            title: "Add exam insertion successful",
            timer: 1500,
          }).then(() => {
            navigate("/exam");
          });
          ...
          Swal.fire({
            title: "Add exam insertion failed",
            icon: "error",
          });
        }
      }
              ...
              <div className="row">
                <nav aria-label="breadcrumb" style={{ '--bs-breadcrumb-divider': 'none' }}>
                  <ol className="breadcrumb">
                    ...
                    <li className="breadcrumb-item active" aria-current="page">Add Exam</li>
                  </ol>
                </nav>
