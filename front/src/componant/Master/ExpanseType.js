...
function ExpanseType() {
  ...
  const handleDelete = async (id) => {
    try {
      const result = await Swal.fire({
        title: "Are you sure?",
        text: "You want to delete this Expanse Type",
        icon: "warning",
      ...
      if (result.isConfirmed) {
        const res = await axios.delete(
          `${process.env.REACT_APP_BACKEND_BASE_URL}/delete-expanse-type/` + id
        );
        if (res.status === 200) {
          getExpansetypeData();
          Swal.fire({
            title: "Deleted!",
            text: "This Expanse Type has been deleted.",
            icon: "success",
          ...
          Swal.fire({
            ...
            text: "Expanse Type deleted failed.",
            icon: "error",
          });
        }
      }
  ...
  const filterDataBySearch = (data) => {
    return data.filter((item) =>
      item.expanse_type.toLowerCase().includes(searchQuery.toLowerCase())
    );
  };
  useEffect(() => {
          ...
          <div className="content">
            <nav aria-label="breadcrumb" style={{ '--bs-breadcrumb-divider': 'none' }}>
              <ol className="breadcrumb">
                ...
                <li className="breadcrumb-item active" aria-current="page">Expense Type</li>
              </ol>
            </nav>
            ...
            <div className="d-flex justify-content-between">
              ...
              <div className="list-btn mt-4">
                <ul>
                  <li>
                    <Link to={"/createexpansetype"} className="btn btn-primary">
                        ...
                        filteredData.map((dataa, index) => {
                          return (
                            <tr key={index}>
                              <td>{index + 1}</td>
                              <td>
                                {dataa.expanse_type.charAt(0).toUpperCase() +
                                  dataa.expanse_type.slice(1)}
                              </td>
                              <td>
                                <div className="table-actions d-flex">
                                  <Link
                                    className="delete-table me-2"
                                    to={`/editexpansetype/${dataa.id}`}
                                  >
                                    {" "}
