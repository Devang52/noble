...
export default function EditReferenceType() {
    ...
    const getReferanceData = async () => {
        try {
            const res = await axios.get(
                `${process.env.REACT_APP_BACKEND_BASE_URL}/getedit-referencetype/${id}`
            );
            if (res.status === 200 && res.data.data.length > 0) {
                setValues({ ...values, name: res.data.data[0].name })
            } else {
                Swal.fire({
                    icon: "error",
                    title: "Referance Type not found",
                });
                navigate("/referencetype");
            }
        } catch (err) {
            console.log(err);
            setLoader(true)
    ...
    const headleUpdate = async (event) => {
        ...
        try {
            const trimmedReferanceType = {
                name: values.name.trim()
            };

            if (!trimmedReferanceType.name) {
                Swal.fire({
                ...
                const res = await axios.put(
                    `${process.env.REACT_APP_BACKEND_BASE_URL}/edit-referencetype/${id}`,
                    trimmedReferanceType
                );

                if (res.status === 200) {
                    Swal.fire({
                        icon: "success",
                        title: "Reference Type Updated Successfully",
                        timer: 1500,
                    }).then(() => {
                        navigate('/referencetype');
                    });
                            ...
                            <div class="row">
                                <nav aria-label="breadcrumb" style={{ '--bs-breadcrumb-divider': 'none' }}>
                                    <ol className="breadcrumb">
                                        ...
                                        <li className="breadcrumb-item"><NavLink to="/referencetype">Reference Type </NavLink>/</li>
                                        <li className="breadcrumb-item active" aria-current="page">Edit Reference Type</li>
                                    </ol>
                                </nav>
                                <div class="col-lg-12 col-sm-12">
                                    <div class="content-page-header">
                                        <h5>Edit Reference Type</h5>
                                    </div>
                                    <div class="row">
                                        <div class="col-lg-6">
                                            <div class="form-group">
                                                <label>Reference Type</label>
                                                <input type="text" class="form-control" placeholder="Enter Reference Type"
                                                    value={values.name}
                                    ...
                                    <div class="col-lg-6">
                                        <div class="btn-path">
                                            ...
                                            <Link to={'/referencetype'} class="btn btn-cancel me-3">Back</Link>
