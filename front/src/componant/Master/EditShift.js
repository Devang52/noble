  ...
  const getShiftData = async () => {
    try {
      const res = await axios.get(
        `${process.env.REACT_APP_BACKEND_BASE_URL}/getedit-shift/${id}`
      );
      if (res.status === 200 && res.data.data.length > 0) {
        setValues({ ...values, shift: res.data.data[0].shift })
      } else {
        Swal.fire({
          icon: "error",
    ...
    try {
      ...
      if (!trimmedShift.shift) {
        ...
        const res = await axios.put(
          `${process.env.REACT_APP_BACKEND_BASE_URL}/edit-shift/${id}`,
          trimmedShift
        );
        ...
        if (res.status === 200 && res.data.data) {
          ...
          Swal.fire({
            title: "Shift Type Updated failed",
            icon: "error",
          });
        }
      }
              ...
              <div className="row">
                ...
                <nav aria-label="breadcrumb" style={{ '--bs-breadcrumb-divider': 'none' }}>
                  <ol className="breadcrumb">
                    ...
                    <li className="breadcrumb-item"><NavLink to="/shift">Shift </NavLink>/</li>
                    <li className="breadcrumb-item active" aria-current="page">Edit Shift</li>
                  </ol>
                </nav>
                {/* {/ <!-- [ breadcrumb ] end --> /} */}
                    ...
                    <div className="col-lg-6">
                      <div className="form-group">
                        ...
                        <div className="btn-path">
                          <Link to={'/shift'} className="btn btn-cancel me-3" >Back</Link>
