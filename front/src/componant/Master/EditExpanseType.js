...
function EditExpanseType() {
    ...
    const getExpanseTypeData = async () => {
        try {
            const res = await axios.get(
                `${process.env.REACT_APP_BACKEND_BASE_URL}/getedit-expanse-type/${id}`
            );
            if (res.status === 200) {
                setValues({ ...values, expanse_type: res.data.data[0].expanse_type })
            } else {
                setValues([]);
            }
        } catch (err) {
    ...
    const headleUpdate = async (event) => {
        event.preventDefault();
        try {
            const trimmedExpanseType = {
                expanse_type: values.expanse_type.trim(),
            };
            if (!trimmedExpanseType.expanse_type) {
                Swal.fire({
                ...
                const res = await axios.put(
                    `${process.env.REACT_APP_BACKEND_BASE_URL}/edit-expanse-type/${id}`,
                    trimmedExpanseType
                );
                if (res.status === 200) {
                    Swal.fire({
                        ...
                        navigate('/expansetype');
                    });
                } else {
                    Swal.fire({
                        title: "Expense type updation failed",
                        icon: "error",
                    });
                }
            }
                            ...
                            <div className="row">
                                <nav aria-label="breadcrumb" style={{ '--bs-breadcrumb-divider': 'none' }}>
                                    <ol className="breadcrumb">
                                        ...
                                        <li className="breadcrumb-item"><NavLink to="/expansetype">Expense Type </NavLink>/</li>
                                        <li className="breadcrumb-item active" aria-current="page">Edit Expense Type</li>
                                    </ol>
                                </nav>
                                    ...
                                    <div className="row">
                                        <div className="col-lg-6">
                                            <div className="form-group">
                                                ...
                                                <input type="text" className="form-control" placeholder="Enter Expense Type"
                                                    value={values.expanse_type}
                                                    onChange={e => setValues({ ...values, expanse_type: e.target.value })} />
                                            </div>
                                            <div className="btn-path">

                                                <Link to={'/expansetype'} className="btn btn-cancel me-3">Back</Link>
...
export default EditExpanseType;