...
const Areaedit = () => {
  ...
  const getAreaData = async () => {
    try {
      const res = await axios.get(
        `${process.env.REACT_APP_BACKEND_BASE_URL}/getarea/${id}`
      );
      if (res.status === 200 && res.data.data.length > 0) {
        setArea({
          area: res.data.data[0].area,
          city_id: res.data.data[0].city_id,
          s_id: res.data.data[0].s_id,
          c_id: res.data.data[0].c_id,
        });
        getStateData(res.data.data[0].c_id);
        getCityData(res.data.data[0].s_id, res.data.data[0].c_id);
      } else {
        Swal.fire({
          icon: "error",
  ...
  const handleUpdate = async (event) => {
    ...
    try {
      const trimmedArea = {
        area: area.area.trim(),
        c_id: typeof area.c_id === 'string' ? area.c_id.trim() : area.c_id,
        s_id: typeof area.s_id === 'string' ? area.s_id.trim() : area.s_id,
        city_id: typeof area.city_id === 'string' ? area.city_id.trim() : area.city_id,
      };
      ...
      if (
        ...
        !trimmedArea.s_id ||
        !trimmedArea.city_id ||
        area.s_id === "-- Select State --" ||
        area.city_id === "-- Select City --"
      ) {
        Swal.fire({
        ...
        const res = await axios.put(
          `${process.env.REACT_APP_BACKEND_BASE_URL}/area/update/${id}`,
          trimmedArea
        );
                  ...
                  <div className="row">
                    <div className="col-lg-6">
                      ...
                      <div className="form-group">
                        ...
                        <div>
                          <select
                            ...
                            className="form-select"
                            required
                            value={area.city_id} // Fix: Set the value to area.city_id
                            onChange={(e) => {
                              const { value } = e.target;
                              setArea((prevState) => ({
                                ...prevState,
                      ...
                      <div className="btn-path">
                        <Link to={"/area"} className="btn btn-cancel me-3">
                          Back
                        </Link>
