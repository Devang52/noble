...
const CityEdit = () => {
  ...
  const handleInputChange = (e) => {
    ...
    if (name === "c_id") {
      ...
      getStateData(selectedCountry.id);
    } else {
      setCity({
        ...city,
        [name]: value,
  ...
  const getCityData = async () => {
    try {
      const res = await axios.get(
        `${process.env.REACT_APP_BACKEND_BASE_URL}/getcity/${id}`
      );
      if (res.status === 200 && res.data.data.length > 0) {
        ...
        getStateData(res.data.data[0].c_id); // Fetch states based on the country initially selected
      } else {
        Swal.fire({
          icon: "error",
  ...
  const handleUpdate = async (event) => {
    ...
    try {
      const trimmedCity = {
        city: city.city.trim(),
        c_id: typeof city.c_id === 'string' ? city.c_id.trim() : city.c_id,
        s_id: typeof city.s_id === 'string' ? city.s_id.trim() : city.s_id,
      };

      if (!trimmedCity.city || !trimmedCity.c_id || !trimmedCity.s_id || trimmedCity.s_id === "-- Select State --") {
        Swal.fire({
        ...
        const res = await axios.put(
          `${process.env.REACT_APP_BACKEND_BASE_URL}/city/update/${id}`,
          trimmedCity
        );
