...
const SubCategoryEdit = () => {
  ...
  const getSubCategoryData = async () => {
    try {
      const res = await axios.get(
        `${process.env.REACT_APP_BACKEND_BASE_URL}/getsubcategory/${id}`
      );
  ...
  const handleUpdate = async (event) => {
    ...
    try {
      const trimmedSubCategory = {
        ...
        cat_id:
          typeof subCategory.cat_id === "string"
            ? subCategory.cat_id.trim()
            : subCategory.cat_id,
      };
      if (!trimmedSubCategory.subcategory_name || !trimmedSubCategory.cat_id) {
        ...
        const res = await axios.put(
          `${process.env.REACT_APP_BACKEND_BASE_URL}/subcategory/update/${id}`,
          subCategory
        );
