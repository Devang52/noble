    ...
    try {
      ...
      if (result.isConfirmed) {
        const res = await axios.delete(
          `${process.env.REACT_APP_BACKEND_BASE_URL}/delete-shift/` + id
        );
        if (res.status === 200) {
          getShiftData();
          Swal.fire({
            title: "Deleted!",
            text: "This shift has been deleted.",
            icon: "success",
          ...
          <div className="content">
            ...
            <nav aria-label="breadcrumb" style={{ '--bs-breadcrumb-divider': 'none' }}>
              <ol className="breadcrumb">
                ...
                <li className="breadcrumb-item active" aria-current="page">Shift</li>
              </ol>
            </nav>
            {/* {/ <!-- [ breadcrumb ] end --> /} */}
            ...
            <div className="d-flex justify-content-between">
              ...
              <div className="list-btn mt-4">
                <ul>
                  <li>
                    <Link to={"/createshift"} className="btn btn-primary">
                        ...
                        filteredData.map((dataa, index) => {
                          return (
                            <tr key={index}>
                              <td>{index + 1}</td>
                              <td>
                                {dataa.shift.charAt(0).toUpperCase() +
                                  dataa.shift.slice(1)}
                              </td>
                              <td>
