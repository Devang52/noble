  ...
  const handleDelete = async (id) => {
    try {
      const result = await Swal.fire({
        title: "Are you sure?",
        text: "You want to delete this ReferenceType",
        icon: "warning",
      ...
      if (result.isConfirmed) {
        const res = await axios.delete(
          `${process.env.REACT_APP_BACKEND_BASE_URL}/delete-referencetype/` + id
        );
        if (res.status === 200) {
          getReferencesData();
          Swal.fire({
            title: "Deleted!",
            text: "This references has been deleted.",
            icon: "success",
          ...
          <div className="content">
            ...
            <nav aria-label="breadcrumb" style={{ '--bs-breadcrumb-divider': 'none' }}>
              <ol className="breadcrumb">
                ...
                <li className="breadcrumb-item active" aria-current="page">Reference Type</li>
              </ol>
            </nav>
            {/* {/ <!-- [ breadcrumb ] end --> /} */}
            ...
            <div className="d-flex justify-content-between">
              ...
              <div class="list-btn mt-4">
                <ul>
                  <li>
                    <Link to={"/createreferencetype"} class="btn btn-primary">
                        ...
                        filteredData.map((dataa, index) => {
                          return (
                            <tr key={index}>
                              <td>{index + 1}</td>
                              <td>
                                {dataa.name.charAt(0).toUpperCase() +
                                  dataa.name.slice(1)}
                              </td>
                              <td>
