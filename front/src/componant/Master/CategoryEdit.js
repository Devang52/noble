...
const CategoryEdit = () => {
    ...
    const getCategoryIdData = async () => {
        try {
            const res = await axios.get(
                `${process.env.REACT_APP_BACKEND_BASE_URL}/getcategory/${id}`
            );
            if (res.status === 200) {
                setCategory({ ...category, category_name: res.data.data[0].category_name });
            } else {
                setCategory([]);
            }
        } catch (err) {
    ...
    const headleSubmit = async (event) => {
        ...
        try {
            ...
            if (!trimmedCategory.category_name) {
                Swal.fire({
                ...
                const res = await axios.put(
                    `${process.env.REACT_APP_BACKEND_BASE_URL}/category/update/${id}`,
                    trimmedCategory
                );
