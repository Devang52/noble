...
const SubCategoryAdd = () => {
  ...
  const handleInputChange = (e) => {
    ...
    if (name === 'cat_id') {
      ...
      // For other inputs (e.g., subcategory_name), update as usual
      setSubCategory((prevSubCategory) => ({
        ...prevSubCategory,
        [name]: value,
      }));
  ...
  const getCategories = async () => {
    try {
      const res = await axios.get(`${process.env.REACT_APP_BACKEND_BASE_URL}/getcategory`);
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const trimmedSubCategory = {
        ...
        cat_id: subCategory.cat_id.trim()
      };
      if (!trimmedSubCategory.subcategory_name || !trimmedSubCategory.cat_id) {
        Swal.fire({
          title: 'Please enter all data',
          icon: 'warning',
        });
      } else {
        const res = await axios.post(`${process.env.REACT_APP_BACKEND_BASE_URL}/subcategory`, subCategory);

        if (res.status === 200) {
          // Successful request
          Swal.fire({
            icon: 'success',
            title: 'Subcategory Inserted Successfully',
            timer: 1500,
          }).then(() => {
          ...
          Swal.fire({
            title: 'Subcategory insertion failed',
            icon: 'error',
          });
        }
      }
                    ...
                    <div className="col-lg-6">
                      ...
                      <div className="form-group">
                        ...
                        <div className="btn-path">
                          <Link to={'/subcategory'} className="btn btn-cancel me-3">
                            Back
                          </Link>
