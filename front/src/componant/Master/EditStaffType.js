...
export default function EditStaffType() {
  ...
  const getStaffTypeData = async () => {
    try {
      const res = await axios.get(
        `${process.env.REACT_APP_BACKEND_BASE_URL}/getedit-staff-type/${id}`
      );
      if (res.status === 200 && res.data.data.length > 0) {
        setValues({ ...values, staff_type: res.data.data[0].staff_type })
      } else {
        Swal.fire({
          icon: "error",
          title: "Staff Type not found",
        });
      }
    } catch (err) {
      console.log(err);
  ...
  const headleUpdate = async (event) => {
    ...
    try {
      const trimmedStaffType = {
        staff_type: values.staff_type.trim(),
      };

      if (!trimmedStaffType.staff_type) {
        Swal.fire({
        ...
        const res = await axios.put(
          `${process.env.REACT_APP_BACKEND_BASE_URL}/edit-staff-type/${id}`,
          trimmedStaffType
        );
        ...
        if (res.status === 200 && res.data.data) {
          ...
          Swal.fire({
            icon: "success",
            title: "Staff Type  updated successfully",
            timer: 1500,
          }).then(() => {
            navigate('/stafftype');
          });
          ...
          Swal.fire({
            title: "Staff Type Updation failed",
            icon: "error",
          });
        }
      }
              ...
              <div className="row">
                ...
                <nav aria-label="breadcrumb" style={{ '--bs-breadcrumb-divider': 'none' }}>
                  <ol className="breadcrumb">
                    ...
                    <li className="breadcrumb-item"><NavLink to="/stafftype">Staff Type </NavLink>/</li>
                    <li className="breadcrumb-item active" aria-current="page">Edit Staff Type</li>
                  </ol>
                </nav>
                {/* {/ <!-- [ breadcrumb ] end --> /} */}
                    ...
                    <div className="col-lg-6">
                      <div className="form-group">
                        ...
                        <input
                          ...
                          onChange={e => setValues({ ...values, staff_type: e.target.value })}
                        />
                        <br />
                        <div className="btn-path">
                          <Link to={'/stafftype'} className="btn btn-cancel me-3" >Back</Link>
