...
export default function StaffType() {
  ...
  const handleDelete = async (id) => {
    try {
      const result = await Swal.fire({
        title: "Are you sure?",
        text: "You want to delete this Staff Type",
        icon: "warning",
      ...
      if (result.isConfirmed) {
        const res = await axios.delete(
          `${process.env.REACT_APP_BACKEND_BASE_URL}/delete-staff-type/` + id
        );
        if (res.status === 200) {
          getStaffData();
          Swal.fire({
            title: "Deleted!",
            text: "This Staff Type has been deleted.",
            icon: "success",
          ...
          Swal.fire({
            ...
            text: "Staff Type deleted failed.",
            icon: "error",
          });
        }
      }
  ...
  const filterDataBySearch = (data) => {
    return data.filter((item) =>
      item.staff_type.toLowerCase().includes(searchQuery.toLowerCase())
    );
  };
  useEffect(() => {
          ...
          <div className="content">
            ...
            <nav aria-label="breadcrumb" style={{ '--bs-breadcrumb-divider': 'none' }}>
              <ol className="breadcrumb">
                ...
                <li className="breadcrumb-item active" aria-current="page">Staff Type</li>
              </ol>
            </nav>
            {/* {/ <!-- [ breadcrumb ] end --> /} */}
            ...
            <div className="d-flex justify-content-between">
              ...
              {/* <div className="list-btn m-4">
                <ul>
                  <li>
                    <Link to={"/createstafftype"} className="btn btn-primary">
                        ...
                        filteredData.map((dataa, index) => {
                          return (
                            <tr key={index}>
                              <td>{index + 1}</td>
                              <td>
                                {dataa.staff_type.charAt(0).toUpperCase() +
                                  dataa.staff_type.slice(1)}
                              </td>
                              <td>
                                <div className="table-actions d-flex">
                                  {/* <Link
                                  className="delete-table me-2"
                                  to={`/editstafftype/${dataa.id}`}
                                >
                                  {" "}
