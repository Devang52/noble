...
const StateAdd = () => {
  ...
  const handleInputChange = (e) => {
    ...
    if (name === 'c_id') {
      ...
      // For other inputs (e.g., state), update as usual
      setState((prevState) => ({
        ...prevState,
        [name]: value,
      }));
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      if (!trimmedState.state || !trimmedState.c_id) {
        ...
        const res = await axios.post(`${process.env.REACT_APP_BACKEND_BASE_URL}/state`, trimmedState);

        if (res.status === 200) {
          Swal.fire({
            icon: 'success',
            title: 'State Inserted Successfully',
            timer: 1500,
          }).then(() => {
            navigate('/state');
          });
              ...
              <div className="row">
                ...
                <nav aria-label="breadcrumb" style={{ '--bs-breadcrumb-divider': 'none' }}>
                  <ol className="breadcrumb">
                    ...
                    <li className="breadcrumb-item active" aria-current="page">Add State</li>
                  </ol>
                </nav>
                {/* {/ <!-- [ breadcrumb ] end --> /} */}
                    ...
                    <div className="col-lg-6">
                      ...
                      <div className="form-group">
                        ...
                        <div className="btn-path">
                          <Link to={'/state'} className="btn btn-cancel me-3">
                            Back
                          </Link>
