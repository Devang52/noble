  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const trimmedArea = {
        ...
        c_id: area.c_id.trim(),
        s_id: area.s_id.trim(),
        city_id: area.city_id.trim(),
      };

      if (!trimmedArea.area || !trimmedArea.c_id || !trimmedArea.s_id || !trimmedArea.city_id || area.s_id === "--select State--" || area.city_id === "--select City--") {
        Swal.fire({
          title: 'Please enter all data',
          icon: 'warning',
        });
      } else {
        const res = await axios.post(`${process.env.REACT_APP_BACKEND_BASE_URL}/area`, trimmedArea);

        if (res.status === 200) {
          // Successful request
          Swal.fire({
            icon: 'success',
            title: 'Area Inserted Successfully',
            timer: 1500,
          }).then(() => {
            navigate('/area');
          });
              ...
              <div className="row">
                ...
                <nav aria-label="breadcrumb" style={{ '--bs-breadcrumb-divider': 'none' }}>
                  <ol className="breadcrumb">
                    ...
                    <li className="breadcrumb-item active" aria-current="page">Add Area</li>
                  </ol>
                </nav>
                {/* {/ <!-- [ breadcrumb ] end --> /} */}
                        ...
                        <div>
                          <select
                            ...
                            onChange={(e) => {
                              ...
                              getCityData(e.target.value, area.c_id, area.s_id);
                              handleInputChange(e);
                            }}
                          >
