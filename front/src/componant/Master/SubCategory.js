...
const SubCategory = () => {
  ...
  const getCategoryData = async () => {
    try {
      const res = await axios.get(
        `${process.env.REACT_APP_BACKEND_BASE_URL}/getcategory`
      );
      if (res.status === 200) {
  ...
  const getsubCategoryData = async () => {
    try {
      const res = await axios.get(
        `${process.env.REACT_APP_BACKEND_BASE_URL}/getallsubcategory`
      );
      if (res.status === 200) {
    ...
    try {
      ...
      if (result.isConfirmed) {
        const res = await axios.delete(
          `${process.env.REACT_APP_BACKEND_BASE_URL}/subcategory/delete/` + id
        );
        if (res.status === 200) {
          getsubCategoryData();
          Swal.fire({
            title: "Deleted!",
            text: "This Sub Category has been deleted.",
            icon: "success",
          ...
          Swal.fire({
            ...
            text: "Sub Category deleted failed.",
            icon: "error",
          });
        }
      }
  ...
  useEffect(() => {
    setFilteredData(filterDataBySearch(subCategory ?? []));
                  ...
                  <table className="table datatable">
                    ...
                    <tbody>
                      {filteredData.length === 0 ? (
                        ...
                        filteredData.map((subcat, index) => {
                          return (
                            <tr key={index}>
                              <td>{index + 1}</td>
                              <td>{getCategoryNameById(subcat.cat_id)}</td>
                              <td>
                                {subcat.subcategory_name.charAt(0).toUpperCase() +
                                  subcat.subcategory_name.slice(1)}
                              </td>
                              <td>
                                <div className="table-actions d-flex">
                                  <NavLink
                                    className="delete-table me-2"
                                    to={`/editsubcategory/${subcat.id}`}
                                  >
