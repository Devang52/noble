...
function EditIncomeType() {
    ...
    const getIncomeTypeData = async () => {
        try {
            const res = await axios.get(
                `${process.env.REACT_APP_BACKEND_BASE_URL}/getedit-income-type/${id}`
            );
            if (res.status === 200) {
                setValues({ ...values, income_type: res.data.data[0].income_type })
            } else {
                setValues([]);
            }
        } catch (err) {
    ...
    const headleUpdate = async (event) => {
        ...
        try {
            const trimmedIncomeType = {
                income_type: values.income_type.trim(),
            };
            if (!trimmedIncomeType.income_type) {
                Swal.fire({
                ...
                const res = await axios.put(
                    `${process.env.REACT_APP_BACKEND_BASE_URL}/edit-income-type/${id}`,
                    trimmedIncomeType
                );
                ...
                if (res.status === 200) {
                    Swal.fire({
                        icon: "success",
                        title: "Income Type Updated Successfully",
                        timer: 1500,
                    }).then(() => {
                        navigate('/incometype');
                    });
                    ...
                    Swal.fire({
                        title: "Income Type updation failed",
                        icon: "error",
                    });
                }
            }
                            ...
                            <div className="row">
                                ...
                                <nav aria-label="breadcrumb" style={{ '--bs-breadcrumb-divider': 'none' }}>
                                    <ol className="breadcrumb">
                                        ...
                                        <li className="breadcrumb-item"><NavLink to="/incometype">Income Type </NavLink>/</li>
                                        <li className="breadcrumb-item active" aria-current="page">Edit Income Type</li>
                                    </ol>
                                </nav>
                                {/* {/ <!-- [ breadcrumb ] end --> /} */}
                                <div className="col-lg-12 col-sm-12">
                                    <div className="content-page-header">
                                        <h5>Edit Income Type</h5>
                                    </div>
                                    <div className="row">
                                        <div className="col-lg-6">
                                            <div className="form-group">
                                                <label>Income Type</label>
                                                <input type="text" className="form-control" placeholder="Enter Income Type"
                                                    value={values.income_type}
                                                    onChange={e => setValues({ ...values, income_type: e.target.value })} />
                                            </div>
                                            <div className="btn-path">

                                                <Link to={'/incometype'} className="btn btn-cancel me-3">Back</Link>
