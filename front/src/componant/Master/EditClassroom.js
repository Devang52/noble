    ...
    const getClassRoomData = async () => {
        try {
            const res = await axios.get(
                `${process.env.REACT_APP_BACKEND_BASE_URL}/getclassroom/${id}`
            );
            if (res.status === 200) {
        ...
        try {
            ...
            if (!trimmedClass.name) {
                ...
                const res = await axios.put(
                    `${process.env.REACT_APP_BACKEND_BASE_URL}/classroom/update/${id}`,
                    trimmedClass
                );
                ...
                if (res.status === 200) {
                    Swal.fire({
                        icon: "success",
                        title: " Class Room Updated Successfully",
                        timer: 1500,
                    }).then(() => {
                        navigate('/class');
                    });
                    ...
                    Swal.fire({
                        title: "Class room updation failed",
                        icon: "error",
                    });
                }
            }
                            ...
                            <div class="row">
                                <nav aria-label="breadcrumb" style={{ '--bs-breadcrumb-divider': 'none' }}>
                                    <ol className="breadcrumb">
                                        ...
                                        <li className="breadcrumb-item"><NavLink to="/class">ClassRoom </NavLink>/</li>
                                        <li className="breadcrumb-item active" aria-current="page">Edit ClassRoom</li>
                                    </ol>
                                </nav>
                                <div class="col-lg-12 col-sm-12">
                                    <div class="content-page-header">
                                        <h5>Edit Class Room</h5>
                                    </div>
                                    <div class="row">
                                        <div class="col-lg-6">
                                            <div class="form-group">
                                                <label>Class Room</label>
                                                <input type="text" class="form-control" placeholder="Enter Class Room"
                                                    value={values.name}
                                                    onChange={e => setValues({ ...values, name: e.target.value })} />
                                            </div>
                                            <div class="btn-path">

                                                <Link to={'/class'} class="btn btn-cancel me-3">Back</Link>
