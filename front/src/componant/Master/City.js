...
const City = () => {
    ...
    try {
      ...
      if (result.isConfirmed) {
        const res = await axios.delete(
          `${process.env.REACT_APP_BACKEND_BASE_URL}/city/delete/` + id
        );
        if (res.status === 200) {
          getCityData();
          Swal.fire({
            title: "Deleted!",
            text: "This City has been deleted.",
            icon: "success",
  ...
  useEffect(() => {
    setFilteredData(filterDataBySearch(city ?? []));
  }, [city, searchQuery]);
                        ...
                        filteredData.map((dataa, index) => {
                          return (
                            <tr key={index}>
                              <td>{index + 1}</td>
                              <td>
                                {dataa.city.charAt(0).toUpperCase() +
                                  dataa.city.slice(1)}
                              </td>
