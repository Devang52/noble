...
const State = () => {
    ...
    try {
      ...
      if (result.isConfirmed) {
        const res = await axios.delete(
          `${process.env.REACT_APP_BACKEND_BASE_URL}/state/delete/` + id
        );
        if (res.status === 200) {
          getStateData();
          Swal.fire({
            title: "Deleted!",
            text: "This State has been deleted.",
            icon: "success",
  ...
  useEffect(() => {
    setFilteredData(filterDataBySearch(state ?? []));
  }, [state, searchQuery]);
  if (loader) {
          ...
          <div className="content">
            ...
            <div className="d-flex justify-content-between">
              ...
              <div className="list-btn mt-4">
                <ul>
                  <li>
                    <Link to={"/addstate"} className="btn btn-primary">
                        ...
                        filteredData.map((dataa, index) => {
                          return (
                            <tr key={index}>
                              <td>{index + 1}</td>
                              <td>
                                {dataa.state.charAt(0).toUpperCase() +
                                  dataa.state.slice(1)}
                              </td>
