...
function IncomeType() {
  ...
  const handleDelete = async (id) => {
    try {
      const result = await Swal.fire({
        title: "Are you sure?",
        text: "You want to delete this Income Type",
        icon: "warning",
      ...
      if (result.isConfirmed) {
        const res = await axios.delete(
          `${process.env.REACT_APP_BACKEND_BASE_URL}/delete-income-type/` + id
        );
        if (res.status === 200) {
          getIncometypeData();
          Swal.fire({
            title: "Deleted!",
            text: "This Income Type has been deleted.",
            icon: "success",
          ...
          Swal.fire({
            ...
            text: "Income Type deleted failed.",
            icon: "error",
          });
        }
      }
  ...
  const filterDataBySearch = (data) => {
    return data.filter((item) =>
      item.income_type.toLowerCase().includes(searchQuery.toLowerCase())
    );
  };
  useEffect(() => {
          ...
          <div className="content">
            ...
            <nav aria-label="breadcrumb" style={{ '--bs-breadcrumb-divider': 'none' }}>
              <ol className="breadcrumb">
                ...
                <li className="breadcrumb-item active" aria-current="page">Income Type</li>
              </ol>
            </nav>
            {/* {/ <!-- [ breadcrumb ] end --> /} */}
            ...
            <div className="d-flex justify-content-between">
              ...
              <div className="list-btn mt-4">
                <ul>
                  <li>
                    <Link to={"/createincometype"} className="btn btn-primary">
                        ...
                        filteredData.map((dataa, index) => {
                          return (
                            <tr key={index}>
                              <td>{index + 1}</td>
                              <td>
                                {dataa.income_type.charAt(0).toUpperCase() +
                                  dataa.income_type.slice(1)}
                              </td>
                              <td>
                                <div className="table-actions d-flex">
                                  <Link
                                    className="delete-table me-2"
                                    to={`/editincometype/${dataa.id}`}
                                  >
                                    {" "}
