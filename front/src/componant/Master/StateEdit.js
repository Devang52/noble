...
const StateEdit = () => {
    ...
    const getStateData = async () => {
        try {
            const res = await axios.get(`${process.env.REACT_APP_BACKEND_BASE_URL}/getstate/${id}`);
    ...
    const handleUpdate = async (event) => {
        ...
        try {
            const trimmedState = {
                ...
                c_id: typeof state.c_id === 'string' ? state.c_id.trim() : state.c_id,
            };

            if (!trimmedState.state || !trimmedState.c_id) {
                Swal.fire({
                    title: "Please enter data",
                    icon: "warning",
                });
            } else {
                const res = await axios.put(`${process.env.REACT_APP_BACKEND_BASE_URL}/state/update/${id}`, trimmedState);
                            ...
                            <div className="col-lg-6">
                                ...
                                <nav aria-label="breadcrumb" style={{ '--bs-breadcrumb-divider': 'none' }}>
                                    <ol className="breadcrumb">
                                        ...
                                        <li className="breadcrumb-item"><NavLink to="/state">State </NavLink>/</li>
                                        <li className="breadcrumb-item active" aria-current="page">Edit State</li>
                                    </ol>
                                </nav>
                                {/* {/ <!-- [ breadcrumb ] end --> /} */}
                                ...
                                <div className="form-group mt-4">
                                    ...
                                    <div className="btn-path">
                                        <NavLink to={'/state'} className="btn btn-cancel me-3">
                                            Back
                                        </NavLink>
