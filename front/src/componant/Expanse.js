...
function Expanse() {
    ...
    try {
      ...
      if (result.isConfirmed) {
        const res = await axios.delete(
          `${process.env.REACT_APP_BACKEND_BASE_URL}/expanse/delete/` + id
        );
        if (res.status === 200) {
          getBranchData();
          getExpensetypeData();
  ...
  const getExpenseTypeNameById = (I_Id) => {
    ...
    return foundExpenseType
      ? foundExpenseType.expanse_type
      : "Unknown ExpenseType";
  };
  ...
  const filterDataBySearch = (data, query) => {
    ...
    return data.filter(
      (item) =>
        item.expense_name &&
        item.expense_name.toLowerCase().includes(query.toLowerCase())
    );
  };

  useEffect(() => {
    setFilteredData(filterDataBySearch(expense ?? [], searchQuery));
  }, [expense, searchQuery]);
          ...
          <div className="content">
            <nav aria-label="breadcrumb" style={{ '--bs-breadcrumb-divider': 'none' }}>
              <ol className="breadcrumb">
                ...
                <li className="breadcrumb-item active" aria-current="page">Expense</li>
              </ol>
            </nav>
            <div className="content-page-header content-page-headersplit">
              ...
              <div className="list-btn">
                <ul>
                  <li>
                    <Link to={"/createexpense"} className="btn btn-primary">
                    ...
                    <tbody>
                      {currentItems.length === 0 ? (
                        ...
                        currentItems.map((expense, index) => (
                          <tr key={index}>
                            <td>{index + 1}</td>
                            <td>{getBranchNameById(expense.id_branch)}</td>
                            <td>{expense.expense_name}</td>
                            <td>
                              {getExpenseTypeNameById(expense.expense_type)}
                            </td>
                            ...
                            <td>
                              <div className="table-actions d-flex">
                                <Link
                                  ...
                                  to={`/editexpanse/${expense.id}`}
                                >
                                  <i className="fas fa-edit"></i>
                                </Link>
                                <NavLink
                                  className="delete-table delll"
                                  onClick={() => handleDelete(expense.id)} // Pass a function reference
                                >
                                  <img
