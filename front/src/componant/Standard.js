...
function Standard() {
    ...
    try {
      ...
      if (res.status === 200) {
        if (userData.roll === "Admin") {
          ...
          setStandard(
            res.data.data.filter((board) => board.branch_id === branchId)
          );
        }
      } else {
        setStandard([]);
  ...
  const handleDelete = async (id) => {
    try {
      const result = await Swal.fire({
        title: "Are you sure?",
        text: "You will be delete this Standard",
        icon: "warning",
      ...
      if (result.isConfirmed) {
        const res = await axios.delete(
          `${process.env.REACT_APP_BACKEND_BASE_URL}/delete-standerd/` + id
        );
        if (res.status === 200) {
          getStandardData();
          Swal.fire({
            title: "Deleted!",
            text: "This Standard has been deleted.",
            icon: "success",
  ...
  const getBranchNameById = (bId) => {
    const foundBranch = branch.find((b_id) => b_id.id === bId);
    return foundBranch ? foundBranch.name : "Unknown Branch";
  };
  const getBoardNameById = (bId) => {
    const foundBoard = board.find((b_id) => b_id.id === bId);
    ...
    if (name === "branch_id") {
      ...
      if (filtered.length === 0) {
        ...
        const selectedBranch = branch.find(
          (branchItem) => branchItem.id === parseInt(value)
        );
        if (selectedBranch) {
          const filteredBoards = board.filter(
            (boardItem) => boardItem.id_branch === selectedBranch.id
          );
          setFilteredBoards(filteredBoards);
          // Reset medium when branch changes
          setFilteredMedium([]);
        }
      }
      ...
      if (filtered.length === 0) {
        ...
        const selectedBoard = board.find(
          (boardItem) => boardItem.id === parseInt(value)
        );
        if (selectedBoard) {
          const filteredMedium = medium.filter(
            (mediumItem) => mediumItem.id_board === selectedBoard.id
          );
          setFilteredMedium(filteredMedium);
        }
      }
  ...
  useEffect(() => {
    setFilteredData(standard);
  }, [standard]);

  const handleSearch = (e) => {
    setSearchQuery(e.target.value);
  };
  const filterDataBySearch = (standard) => {
    return standard.filter((item) => {
      const branchName = getBranchNameById(item.branch_id).toLowerCase();
      const boardName = getBoardNameById(item.id_board).toLowerCase();
      const standard = item.standard.toLowerCase();
      const searchQueryLower = searchQuery.toLowerCase();

      // Check if branch name, board name, or medium includes search query
      return (
        branchName.includes(searchQueryLower) ||
        boardName.includes(searchQueryLower) ||
        standard.includes(searchQueryLower)
      );
    });
  };

  useEffect(() => {
    setFilteredData(filterDataBySearch(standard));
  }, [standard, searchQuery]);
          ...
          <div class="content">
            ...
            <div class="content-page-header content-page-headersplit">
              ...
              <div class="list-btn">
                <ul>
                  <li>
                    <Link to={"/createstandard"} class="btn btn-primary">
                  ...
                  <table class="table datatable">
                    ...
                    <tbody>
                      {filteredData.length === 0 ? (
                        ...
                        filteredData.map((stand, index) => {
                          return (
                            <tr key={index}>
                              <td>{index + 1}</td>
                              <td>{stand.standard}</td>
                              <td>{stand.branch_name}</td>
                              <td>{stand.board_name}</td>
                              <td>{stand.medium_name}</td>
                              <td>
                                <div className="table-actions d-flex">
                                  <Link
                                    className="delete-table me-2"
                                    to={`/editstandard/${stand.id}`}
                                  >
                                    <i className="fas fa-edit"></i>
                                  </Link>
                                  <NavLink
                                    className="delete-table delll"
                                    onClick={() => handleDelete(stand.id)} // Pass a function reference
                                  >
                                    <img
