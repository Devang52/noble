    ...
    try {
      ...
      if (result.isConfirmed) {
        const res = await axios.put(
          `${process.env.REACT_APP_BACKEND_BASE_URL}/deleteApproval/` + id
        );
        if (res.status === 200) {
          Swal.fire({
            title: "Send Request!",
            text: "Student removal request sent to admin for approval.",
            icon: "success",
                              ...
                              <td>
                                <div className="table-actions d-flex">
                                  <Link
                                    ...
                                    to={`/studentdetails/${dataa.id}`}
                                  >
                                    <i class="fa-solid fa-eye"></i>
                                  </Link>
                                  <Link
                                    className="delete-table me-2"
                                    to={`/editstudent/${dataa.id}`}
                                  >
                                    {" "}
