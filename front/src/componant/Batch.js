...
export default function Batch() {
    ...
    try {
      ...
      if (result.isConfirmed) {
        const res = await axios.delete(
          `${process.env.REACT_APP_BACKEND_BASE_URL}/batch/delete/` + id
        );
        if (res.status === 200) {
          getBatchData();
          Swal.fire({
            title: "Deleted!",
            text: "This batch has been deleted.",
            icon: "success",
  ...
  const handleDropdownChange = async (e) => {
    ...
    if (name === "id_branch") {
      const selectedBranchId = parseInt(value);
      const filteredBoards = board.filter(boardItem => boardItem.id_branch === selectedBranchId);
      setFilteredBoards(filteredBoards);

      const filteredData = selectedBranchId !== "" ? data.filter(item => item.id_branch === selectedBranchId) : [];
      setFilteredData(filteredData);

      // Reset medium and standard
      setFilteredMedium([]);
      setFilteredStandard([]);
    } else if (name === "id_board") {
      const selectedBoardId = parseInt(value);
      const filteredMedium = medium.filter(mediumItem => mediumItem.id_board === selectedBoardId);
      setFilteredMedium(filteredMedium);

      const filteredData = selectedBoardId !== "" ? data.filter(item => item.id_board === selectedBoardId) : [];
      setFilteredData(filteredData);
      ...
      const filteredStandard = standard.filter(standardItem => standardItem.id_medium === selectedMediumId);
      setFilteredStandard(filteredStandard);

      const filteredData = selectedMediumId !== "" ? data.filter(item => item.id_medium === selectedMediumId) : [];
      setFilteredData(filteredData);
    } else if (name === "id_standard") {
      const selectedStandardId = parseInt(value);
      const filteredData = selectedStandardId !== "" ? data.filter(item => item.id_standard === selectedStandardId) : [];
      setFilteredData(filteredData);
    } else {
      setFilteredData([]);
    }
          ...
          <div className="content">
            <nav aria-label="breadcrumb" style={{ '--bs-breadcrumb-divider': 'none' }}>
              <ol className="breadcrumb">
                ...
                <li className="breadcrumb-item active" aria-current="page">Batch</li>
              </ol>
            </nav>
            <div className="content-page-header content-page-headersplit">
              ...
              <div className="list-btn">
                <ul>
                  <li>
                    <Link to={"/createbatch"} className="btn btn-primary">
                        ...
                        currentItems.map((dataa, index) => {
                          return (
                            <tr key={index}>
                              ...
                              <td>{dataa.shift_name}</td>
                              <td>
                                <div className="table-actions d-flex">
                                  <Link
                                    className="delete-table me-2"
                                    to={`/editbatch/${dataa.id}`}
                                  >
                                    {" "}
