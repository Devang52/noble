...
const EditAssignHomework = () => {
  ...
  const getAssignHomeworkData = async () => {
    try {
      const res = await axios.get(
        `${process.env.REACT_APP_BACKEND_BASE_URL}/getedit/assignHomeWork/${id}`
      );
    ...
    try {
      ...
      if (
        ...
        const res = await axios.put(
          `${process.env.REACT_APP_BACKEND_BASE_URL}/assignHomeWork/update/${id}`,
          formDataObject
        );
        if (res.status === 200) {
          Swal.fire({
            icon: "success",
            title: "Assign Homework updated successfully",
            timer: 1500,
          }).then(() => {
            navigate("/assignhomework");
          });
          ...
          Swal.fire({
            title: "Assign Homework updation failed",
            icon: "error",
          });
        }
      }
              ...
              <div className="row">
                ...
                <nav aria-label="breadcrumb" style={{ '--bs-breadcrumb-divider': 'none' }}>
                  <ol className="breadcrumb">
                    ...
                    <li className="breadcrumb-item"><NavLink to="/assignhomework">Assign HomeWork </NavLink>/</li>
                    <li className="breadcrumb-item active" aria-current="page">Edit AssignHomeWork</li>
                  </ol>
                </nav>
                {/* {/ <!-- [ breadcrumb ] end --> /} */}
                <div className="col-lg-12 col-sm-12">
                  <div className="content-page-header">
                    <h5>Edit AssignHomeWork</h5>
                  </div>
                  <div className="row">
                    <div className="col-6">
...
export default EditAssignHomework;
