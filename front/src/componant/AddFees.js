...
function AddFees() {
  ...
  const getStudentfees = async (ids) => {
    try {
      const res = await axios.get(
        `${process.env.REACT_APP_BACKEND_BASE_URL}/getFeesbyStudentTotal/${ids}`
      );
      if (res.status === 200) {
        const fees = res.data.data.final_fees
        setStudentFee(fees);
        setValues({ ...values, amount: fees, id_student: ids })
      } else {
        setStudentFee([]);
      }
    } catch (err) {
  ...
  const handleInputChange = (e) => {
    ...
    //   if (selectedCourse) {
    //     const amount = selectedCourse.fees;
  ...
  const headleSubmit = async (event) => {
    ...
    try {
      const trimmedValues = {
        id_branch: values.id_branch.trim(),
        amount: values.amount,
        id_student: values.id_student.trim(),
      };

      if (!trimmedValues.id_branch || !trimmedValues.amount || !trimmedValues.id_student) {
        Swal.fire({
        ...
        const res = await axios.post(
          `${process.env.REACT_APP_BACKEND_BASE_URL}/create/fees`,
          trimmedValues
        );
          ...
          <div className="content">
            <nav aria-label="breadcrumb" style={{ '--bs-breadcrumb-divider': 'none' }}>
              <ol className="breadcrumb">
                ...
                <li className="breadcrumb-item active" aria-current="page">Add Fees</li>
              </ol>
            </nav>
            <form onSubmit={headleSubmit}>
              ...
              <div className="row">
                ...
                <div className="btn-path">
                  <NavLink to="/fees" className="btn btn-cancel me-3">
                    Back
                  </NavLink>
