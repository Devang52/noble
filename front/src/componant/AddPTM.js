    ...
    try {
      if (
        ...
        const res = await axios.post(
          `${process.env.REACT_APP_BACKEND_BASE_URL}/createptm`,
          values
        );
        if (res.status === 200) {
          // Successful request
    ...
    if (name == "allstudent" || name === "student") {
      setValues((prevValues) => {
        if (checked) {
          return {
            ...prevValues,
            id_student: [...prevValues.id_student, studentId],
          };
        } else {
          return {
            ...prevValues,
            id_student: prevValues.id_student.filter((id) => id !== studentId),
          };
        }
      });
    } else {
              ...
              <div className="row">
                <nav aria-label="breadcrumb" style={{ '--bs-breadcrumb-divider': 'none' }}>
                  <ol className="breadcrumb">
                    ...
                    <li className="breadcrumb-item active" aria-current="page">Add PTM</li>
                  </ol>
                </nav>
