  ...
  useEffect(() => {
    if (userData) {
      if (userData.roll === "Admin") {
        ...
        getStandardFilterData(branchId)
  ...
  const getPTMData = async () => {
    try {
      const res = await axios.get(
        `${process.env.REACT_APP_BACKEND_BASE_URL}/getptm`
      );
      if (res.status === 200) {
        if (userData.roll === 'Admin') {
          setData(res.data.data);
        } else {
          // const branchFiltered = res.data.data.filter(notice => {
          //   const noticeBranchIds = notice.id_branch.split(',').map(id => id.trim());
          //   return noticeBranchIds.includes(branchId.toString());
          // });
          // setData(res.data.data);
          setData(
  ...
  const handleDelete = async (id) => {
    try {
      const result = await Swal.fire({
        title: "Are you sure?",
        text: "You want to delete this PTM",
        icon: "warning",
      ...
      if (result.isConfirmed) {
        const res = await axios.delete(`${process.env.REACT_APP_BACKEND_BASE_URL}/deleteptm/` + id);
        if (res.status === 200) {
          getPTMData();
          Swal.fire({
            title: "Deleted!",
            text: "This PTM has been deleted.",
            icon: "success",
            ...
            <div className="content">
              ...
              <nav aria-label="breadcrumb" style={{ '--bs-breadcrumb-divider': 'none' }}>
                <ol className="breadcrumb">
                  ...
                  <li className="breadcrumb-item active" aria-current="page">PTM</li>
                </ol>
              </nav>
              {/* {/ <!-- [ breadcrumb ] end --> /} */}
              <div className="content-page-header content-page-headersplit">
                ...
                <div className="list-btn">
                  <ul>
                    <li>
                      <Link to={"/addptm"} className="btn btn-primary">
                      ...
                      <tbody>
                        {currentItems.length === 0 ? (
                          ...
                          currentItems.map((data, index) => {
                            // const branchIdsString = data.id_branch;
                            // const matchingBranchIds = branchIdsString.split(",").map((id) => +id);
                            // const branchNames = matchingBranchIds.map((id) => {
                            //   const matchingBranch = branch.find((b) => b.id === id);
                            //   return matchingBranch ? matchingBranch.name : 'N/A';
                            // });

                            // const standardIdsString = data.id_standard;
                            // const matchingStandardIds = standardIdsString.split(",").map((id) => +id);
                            // const standardNames = matchingStandardIds.map((id) => {
                            //   const matchingStandardIds = standard.find((b) => b.id === id);
                            //   return matchingStandardIds ? matchingStandardIds.standard : 'N/A';
                            // });


                            // const batchIdsString = data.id_batch;
                            // const matchingBatchIds = batchIdsString.split(",").map((id) => +id);
                            // const batchNames = matchingBatchIds.map((id) => {
                            //   const matchingBatchIds = batch.find((b) => b.id === id);
                            //   return matchingBatchIds ? matchingBatchIds.name : 'N/A';
                            // });
                              ...
                              <tr key={index}>
                                ...
                                <td>
                                  <div className="table-actions d-flex">
                                    <Link className="delete-table me-2" to={`/editptm/${data.id}`}> <i className="fas fa-edit"></i> </Link>
                                    <div className="delete-table delll" onClick={() => handleDelete(data.id)}><img src="assets/img/icons/delete.svg" alt="svg" /></div>
                                  </div>
                                </td>
                              </tr>
                            );
