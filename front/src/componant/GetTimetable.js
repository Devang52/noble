...
const GetTimetable = () => {
  ...
  const getMondayData = async () => {
    try {
      const res = await axios.get(
        `${process.env.REACT_APP_BACKEND_BASE_URL}/getTimetableMonday`
      );
      if (res.status === 200) {
        if (userData.roll === "Admin") {
          setData(res.data.data);
        } else {
          setData(res.data.data.filter((item) => item.branch_id === branchId));
          console.log(data);
        }
      } else {
        setData([]);
  ...
  const getTuesdayData = async () => {
    try {
      const res = await axios.get(
        `${process.env.REACT_APP_BACKEND_BASE_URL}/getTimetableTuesday`
      );
      if (res.status === 200) {
        if (userData.roll === "Admin") {
          setTuesday(res.data.data);
        } else {
          setTuesday(res.data.data.filter((item) => item.branch_id === branchId));
        }
      } else {
        setTuesday([]);
      }
  ...
  const getWednesdayData = async () => {
    try {
      const res = await axios.get(
        `${process.env.REACT_APP_BACKEND_BASE_URL}/getTimetableWednesday`
      );
      if (res.status === 200) {
        if (userData.roll === "Admin") {
          setWednesday(res.data.data);
        } else {
          setWednesday(res.data.data.filter((item) => item.branch_id === branchId));
        }
      } else {
        setWednesday([]);
      }
  ...
  const getThursdayData = async () => {
    try {
      const res = await axios.get(
        `${process.env.REACT_APP_BACKEND_BASE_URL}/getTimetableThursday`
      );
      if (res.status === 200) {
        if (userData.roll === "Admin") {
          setThursday(res.data.data);
        } else {
          setThursday(
            res.data.data.filter((item) => item.branch_id === branchId)
          );
        }
      } else {
        setThursday([]);
  ...
  const getFridayData = async () => {
    try {
      const res = await axios.get(
        `${process.env.REACT_APP_BACKEND_BASE_URL}/getTimetableFriday`
      );
      if (res.status === 200) {
        if (userData.roll === "Admin") {
          setFriday(res.data.data);
        } else {
          setFriday(
            res.data.data.filter((item) => item.branch_id === branchId)
          );
  ...
  const getSaturdayData = async () => {
    try {
      const res = await axios.get(
        `${process.env.REACT_APP_BACKEND_BASE_URL}/getTimetableSaturday`
      );
      if (res.status === 200) {
        if (userData.roll === "Admin") {
          setSaturday(res.data.data);
        } else {
          setSaturday(
            res.data.data.filter((item) => item.branch_id === branchId)
          );
  ...
  const getSundayData = async () => {
    try {
      const res = await axios.get(
        `${process.env.REACT_APP_BACKEND_BASE_URL}/getTimetableSunday`
      );
      if (res.status === 200) {
        if (userData.roll === "Admin") {
          setSunday(res.data.data);
        } else {
          setSunday(
            res.data.data.filter((item) => item.branch_id === branchId)
          );
  ...
  const getStandardFilterData = async (branch_id) => {
    try {
      const res = await axios.get(
        `${process.env.REACT_APP_BACKEND_BASE_URL}/standardbybranch/?branch_id=${branch_id}`
      );
      if (res.status === 200) {
  ...
  const getBatchFilterData = async (
    id_branch,
    id_standard
  ) => {
    try {
      const res = await axios.get(
        `${process.env.REACT_APP_BACKEND_BASE_URL}/getbatchbybrst/?id_branch=${id_branch}&id_standard=${id_standard}`
      );
      if (res.status === 200) {
  ...
  const handleDropdownChange = async (e) => {
    ...
    if (name === "branch_id") {
      ...
      if (filtered.length === 0) {
        ...
        await getStandardFilterData(value);
      }
    }
    else if (name === "standard_id") {
      let filtered = data;
  ...
  const handleDelete = async (id, dayOfWeek) => {
    try {
      const result = await Swal.fire({
        title: "Are you sure?",
        text: `You will delete this ${dayOfWeek} Timetable`,
        icon: "warning",
      ...
      if (result.isConfirmed) {
        const res = await axios.delete(
          `${process.env.REACT_APP_BACKEND_BASE_URL}/delete${dayOfWeek}/${id}`
        );
        if (res.status === 200) {
          ...
          Swal.fire({
            title: "Deleted!",
            text: `This ${dayOfWeek} Timetable has been deleted.`,
            icon: "success",
          ...
          Swal.fire({
            ...
            text: `${dayOfWeek} timetable deletion failed.`,
            icon: "error",
          });
        }
      }
  ...
  // Example usage:
  const handleDeleteMonday = async (id) => {
    await handleDelete(id, 'monday');
  };

  const handleDeleteTuesday = async (id) => {
    await handleDelete(id, 'tuesday');
  };
  const handleDeleteWednesday = async (id) => {
    await handleDelete(id, 'wednesday');
  };
  const handleDeleteThursday = async (id) => {
    await handleDelete(id, 'thursday');
  };
  const handleDeleteFriday = async (id) => {
    await handleDelete(id, 'friday');
  };
  const handleDeleteSaturday = async (id) => {
    await handleDelete(id, 'saturday');
  };
  const handleDeleteSunday = async (id) => {
    await handleDelete(id, 'sunday');
  };
                        ...
                        filteredData.map((dataa, index) => {
                          return (
                            <tr key={index}>
                              <td>{index + 1}</td>
                              <td>{dataa.branch}</td>
                              <td>{dataa.standard}</td>
                              <td>{dataa.batch_id}</td>
                              <td>{dataa.subject}</td>
                              <td>{dataa.class}</td>
                              <td>{dataa.teacher}</td>
                              <td>{dataa.time_from}</td>
                              <td>{dataa.time_to}</td>
                              <td>
                                <div className="table-actions d-flex">
                                  <div
                                    className="delete-table delll"
                                    onClick={() => handleDeleteMonday(dataa.id)}
                                  >
                                    <img
                            ...
                            <tr key={index}>
                              ...
                              <td>
                                <div className="table-actions d-flex">
                                  <div
                                    className="delete-table delll"
                                    onClick={() => handleDeleteTuesday(dataa.id)}
                                  >
                                    <img
                                      src="assets/img/icons/delete.svg"
                                      alt="svg"
                            ...
                            <tr key={index}>
                              ...
                              <td>
                                <div className="table-actions d-flex">
                                  <div
                                    className="delete-table delll"
                                    onClick={() => handleDeleteWednesday(dataa.id)}
                                  >
                                    <img
                                      src="assets/img/icons/delete.svg"
                                      alt="svg"
                            ...
                            <tr key={index}>
                              ...
                              <td>
                                <div className="table-actions d-flex">
                                  <div
                                    className="delete-table delll"
                                    onClick={() => handleDeleteThursday(dataa.id)}
                                  >
                                    <img
                                      src="assets/img/icons/delete.svg"
                                      alt="svg"
                            ...
                            <tr key={index}>
                              ...
                              <td>
                                <div className="table-actions d-flex">
                                  <div
                                    className="delete-table delll"
                                    onClick={() => handleDeleteFriday(dataa.id)}
                                  >
                                    <img
                                      src="assets/img/icons/delete.svg"
                                      alt="svg"
                            ...
                            <tr key={index}>
                              ...
                              <td>
                                <div className="table-actions d-flex">
                                  <div
                                    className="delete-table delll"
                                    onClick={() => handleDeleteSaturday(dataa.id)}
                                  >
                                    <img
                                      src="assets/img/icons/delete.svg"
                                      alt="svg"
                            ...
                            <tr key={index}>
                              ...
                              <td>
                                <div className="table-actions d-flex">
                                  <div
                                    className="delete-table delll"
                                    onClick={() => handleDeleteSunday(dataa.id)}
                                  >
                                    <img
                                      src="assets/img/icons/delete.svg"
                                      alt="svg"
