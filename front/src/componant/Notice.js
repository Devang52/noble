...
export default function Notice() {
  ...
  const getNoticeData = async () => {
    try {
      const res = await axios.get(
        `${process.env.REACT_APP_BACKEND_BASE_URL}/notice`
      );
      ...
      if (res.status === 200) {
        if (userData.roll === "Admin") {
          ...
          const branchFilteredNotices = res.data.data.filter((notice) => {
            const noticeBranchIds = notice.id_branch
              .split(",")
              .map((id) => id.trim());
            return noticeBranchIds.includes(branchId.toString());
          });
          setNotice(branchFilteredNotices);
        }
      } else {
        setNotice([]);
      }
    ...
    try {
      ...
      if (res.status === 200) {
        const data =
          userData.roll === "Admin"
            ? res.data.data
            : res.data.data.filter((branch) => branch.id === branchId);
        setBranch(data);
      } else {
        setBranch([]);
      }
    ...
    try {
      ...
      if (result.isConfirmed) {
        const res = await axios.delete(
          `${process.env.REACT_APP_BACKEND_BASE_URL}/notice/delete/` + id
        );
        if (res.status === 200) {
          getNoticeData();
          Swal.fire({
  ...
  const handleDropdownChange = async (e) => {
    ...
    if (name === "id_branch") {
      ...
      setFilteredData(notice); // Reset to show all data if other dropdowns are changed
    }
  };
  useEffect(() => {
    setFilteredData(notice);
  }, [notice]);
  const handleSearch = (e) => {
    setSearchQuery(e.target.value);
  };

  const filterDataBySearch = (notice) => {
  ...
  useEffect(() => {
    setFilteredData(filterDataBySearch(notice));
  }, [notice, searchQuery]);
          ...
          <div className="content">
            ...
            <div className="content-page-header content-page-headersplit">
              ...
              <div className="list-btn">
                <ul>
                  <li>
                    <Link to={"/addnotice"} className="btn btn-primary">
                        ...
                        currentItems.map((notice, index) => {
                          return (
                            <tr key={index}>
                              <td>{index + 1}</td>
                              <td>{notice.name}</td>
                              <td>{notice.branch_name}</td>
                              <td>{notice.board}</td>
                              <td>{notice.medium}</td>
                              <td>{notice.standard_name}</td>
                              <td>{notice.batch_name}</td>
                              <td>{notice.description}</td>
                              <td>
                                <Link to={`${process.env.REACT_APP_BACKEND_BASE_URL}/uploads/${notice.files}`} target="_blank">
                              ...
                              <td>
                                <div className="table-actions d-flex">
                                  {/* <Link className="delete-table me-2" to={`/viewnotice/${notice.id}`}>
                                                                <i class="fa-solid fa-eye"></i>
                                                            </Link> */}
                                  <NavLink
                                    className="delete-table delll"
                                    onClick={() => handleDelete(notice.id)} // Pass a function reference
                                  >
                                    <img
