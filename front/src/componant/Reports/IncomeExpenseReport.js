...
const IncomeExpenseReport = () => {
  ...
  const [fees, setFees] = useState([]); // Define fees state
  ...
  const [currentPageExp, setCurrentPageExp] = useState(0); // Start from page 1
  const [itemsPerPageExp, setItemsPerPageExp] = useState(10);
  ...
  const getIncometypeData = async () => {
    try {
      const res = await axios.get(`${process.env.REACT_APP_BACKEND_BASE_URL}/income-type`);
      if (res.status === 200) {
  ...
  const getIncomeData = async () => {
    try {
      const res = await axios.get(`${process.env.REACT_APP_BACKEND_BASE_URL}/getincome`);
      if (res.status === 200) {
        if (userData.roll === 'Admin') {
          setIncome(res.data.data);
        } else {
          setIncome(res.data.data.filter(board => board.id_branch === branchId));
        }
      } else {
        setIncome([]);
      }
  ...
  const getFeesData = async () => {
    try {
      const res = await axios.get(`${process.env.REACT_APP_BACKEND_BASE_URL}/totalfees`);
      if (res.status === 200) {
        setFees(res.data.data);
      } else {
        setFees(0); // Set fees to 0 or any default value if response status is not 200
      }
    } catch (err) {
      console.log(err);
      setLoader(true)
  ...
  const getExpensetypeData = async () => {
    try {
      const res = await axios.get(
        `${process.env.REACT_APP_BACKEND_BASE_URL}/expanse-type`);
      if (res.status === 200) {
        setExpenseType(res.data.data)
      } else {
        setExpenseType([]);
      }
    } catch (err) {
  ...
  const getExpenseData = async () => {
    try {
      const res = await axios.get(
        `${process.env.REACT_APP_BACKEND_BASE_URL}/getexpanse`);
      if (res.status === 200) {
        if (userData.roll === 'Admin') {
          setExpense(res.data.data);
        } else {
          setExpense(res.data.data.filter(board => board.id_branch === branchId));
        }
      } else {
        setExpense([]);
      }
  ...
  const getIncomeTypeNameById = (I_Id) => {
    const foundIncomeType = incomeType.find((i) => i.id === I_Id);
    return foundIncomeType ? foundIncomeType.income_type : "Unknown IncomeType";
  };

  const getExpenseTypeNameById = (I_Id) => {
    const foundExpenseType = expenseType.find((iId) => iId.id === I_Id);
    return foundExpenseType ? foundExpenseType.expanse_type : 'Unknown ExpenseType';
  };
  ...
  const handleDropdownChange = async (e) => {
    ...
    if (name === "id_branch") {
      ...
      if (value !== "") {
        filteredIncome = filteredIncome.filter((item) => String(item[name]) === value);
        filteredExpense = filteredExpense.filter((item) => String(item[name]) === value);
      }
  ...
  useEffect(() => {
    const filteredIncomeData = income.filter((item) =>
      item.income_title.toLowerCase().includes(searchQuery.toLowerCase())
    );
    setFilteredIncomeData(filteredIncomeData);

    const filteredExpenseData = expense.filter((item) =>
      item.expense_name.toLowerCase().includes(searchQuery.toLowerCase())
    );
    setFilteredExpenseData(filteredExpenseData);
  }, [income, expense, searchQuery]);


  const indexOfLastItem = (currentPage + 1) * itemsPerPage;
  const indexOfFirstItem = indexOfLastItem - itemsPerPage;
  const currentItems = filteredIncomeData.slice(indexOfFirstItem, indexOfLastItem);

  const pageCount = Math.ceil(filteredIncomeData.length / itemsPerPage);
  ...
  const indexOfLastItemExp = (currentPageExp + 1) * itemsPerPageExp;
  const indexOfFirstItemExp = indexOfLastItemExp - itemsPerPageExp;
  const currentItemsExp = filteredExpenseData.slice(indexOfFirstItemExp, indexOfLastItemExp);

  const pageCountExp = Math.ceil(filteredExpenseData.length / itemsPerPageExp);

  const handlePageClickExp = ({ selected }) => {
    setCurrentPageExp(selected);
  };

  const handleItemsPerPageChangeExp = (e) => {
    setItemsPerPageExp(parseInt(e.target.value));
    setCurrentPageExp(0); // Reset current page when items per page changes
  };
  if (loader) {
          ...
          <div className="content">
            <nav aria-label="breadcrumb" style={{ '--bs-breadcrumb-divider': 'none' }}>
              <ol className="breadcrumb">
                ...
                <li className="breadcrumb-item active" aria-current="page">Income Expense Report</li>
              </ol>
            </nav>
                    ...
                    <tbody>
                      {currentItems.length === 0 ? (
                        ...
                        currentItems.map((data, index) => (
                          <tr key={index}>
                            <td>{index + 1}</td>
                            <td>{getBranchNameById(data.id_branch)}</td>
                            <td>{data.income_title}</td>
                            <td>{getIncomeTypeNameById(data.income_type)}</td>
                            <td>{data.amount}</td>
                          </tr>
                        ))
                      )}
              ...
              <div className="col-12">
                <div className="table-responsive">
                  <table className="table datatable">
                    ...
                    <tbody>
                      {currentItemsExp.length === 0 ? (
                        <tr>
                          <td colSpan="5">No data available</td>
                        </tr>
                      ) : (
                        currentItemsExp.map((data, index) => (
                          <tr key={index}>
                            <td>{index + 1}</td>
                            <td>{getBranchNameById(data.id_branch)}</td>
                            <td>{data.expense_name}</td>
                            <td>{getExpenseTypeNameById(data.expense_type)}</td>
                            <td>{data.amount}</td>
                          </tr>
                        ))
                      )}
            ...
            <nav className="mt-4 pagination justify-content-end gap-2">
              <select
                className="form-select" style={{ width: "70px", padding: "0 13px" }}
                value={itemsPerPageExp}
                onChange={handleItemsPerPageChangeExp}
              >
                <option value="10">10</option>
                <option value="15">15</option>
                <option value="20">20</option>
              ...
              <ReactPaginate
                ...
                previousLabel={"Previous"}
                nextLabel={"Next"}
                pageCount={pageCountExp}
                pageClassName={"page-link"}
                onPageChange={handlePageClickExp}
                containerClassName={"pagination"}
                previousLinkClassName={"page-link"}
                nextLinkClassName={"page-link"}
                disabledClassName={"disabled"}
