...
const EditMathsReport = () => {
  ...
  const getAddMathsData = async () => {
    try {
      const res = await axios.get(
        `${process.env.REACT_APP_BACKEND_BASE_URL}/getbyidmathsreport/${id}`
      );
      if (res.status === 200) {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const res = await axios.put(
        `${process.env.REACT_APP_BACKEND_BASE_URL}/editmathsreport/${id}`,
        data
      );
      ...
      if (res.status === 200) {
        ...
        Swal.fire({
          icon: "success",
          title: "Maths Report updated successfully",
          timer: 1500,
        }).then(() => {
        ...
        Swal.fire({
          title: "Maths Report update failed",
          icon: "error",
        });
      }
    } catch (err) {
  ...
  const handleDelete = async (id) => {
    try {
      const result = await Swal.fire({
        title: "Are you sure?",
        text: "You will be delete this Maths Report",
        icon: "warning",
      ...
      if (result.isConfirmed) {
        const res = await axios.delete(
          `${process.env.REACT_APP_BACKEND_BASE_URL}/deletemathsreport/` + id
        );
        if (res.status === 200) {
          if (data.length === 1) {
            setData([]);
          ...
          Swal.fire({
            title: "Deleted!",
            text: "This Maths Report has been deleted.",
            icon: "success",
          ...
          Swal.fire({
            ...
            text: "Maths Report deleted failed.",
            icon: "error",
          });
        }
      }
          ...
          <div className="content">
            <nav aria-label="breadcrumb" style={{ '--bs-breadcrumb-divider': 'none' }}>
              <ol className="breadcrumb">
                ...
                <li className="breadcrumb-item active" aria-current="page">Edit Maths Report</li>
              </ol>
            </nav>
            <div className="content-page-header content-page-headersplit">
              <h5>Edit Maths Report</h5>
            </div>
            <div className="row">
              <div className="col-lg-4">
                <div className="form-group">
                  <label>Branch</label>
                  <select name="id_branch" className="form-select" disabled>
                    <option value="">{filterData.branch_name}</option>
                  </select>
                </div>
              </div>
              <div className="col-lg-4">
                <div className="form-group">
                  <label>Batch</label>
                  <div>
                    <select name="id_batch" className="form-select" disabled>
                      <option value="">{filterData.batch_name}</option>
                    </select>
                  </div>
                </div>
              </div>
              ...
              <div className="col-12">
                <div className="table-responsive">
                  <table className="table datatable">
                    <thead>
                      <tr>
                        <th style={{ padding: '15px 40px' }}>Index</th>
                        <th style={{ padding: '15px 40px' }}>Student Name</th>
                        <th style={{ padding: '15px 40px' }}>Total Marks</th>
                        <th style={{ padding: '15px 40px' }}>Simple Plus (+)</th>
                        <th style={{ padding: '15px 40px' }}>Point Plus (+)</th>
                        <th style={{ padding: '15px 40px' }}>Simple Minus ( - )</th>
                        <th style={{ padding: '15px 40px' }}>Point Minus ( - )</th>
                        <th style={{ padding: '15px 40px' }}>Simple Multiplied by ( x )</th>
                        <th style={{ padding: '15px 40px' }}>Point Multiplied by ( x )</th>
                        <th style={{ padding: '15px 40px' }}>Simple Divided by ( ÷ )</th>
                        <th style={{ padding: '15px 40px' }}>Point Divided by ( ÷ )</th>
                        <th style={{ padding: '15px 40px' }}>Percent ( % )</th>
                        <th style={{ padding: '15px 40px' }}>2√x</th>
                        <th style={{ padding: '15px 40px' }}>3√x</th>
                        <th style={{ padding: '15px 40px' }}>( a + b )</th>
                        <th style={{ padding: '15px 40px' }}>( a² + b² )</th>
                        <th style={{ padding: '15px 40px' }}>ax² + bx + c</th>
                        <th style={{ padding: '15px 40px' }}>( a + b )( b + c )</th>
                        <th style={{ padding: '15px 40px' }}>Simplification</th>
                        <th style={{ padding: '15px 40px' }}>Tables</th>
                        <th style={{ padding: '15px 40px' }}>Marks</th>
                      </tr>
                    </thead>
                    <tbody>
                      {data.length === 0 ? (
                        ...
                        data.map((dataa, index) => (
                          <tr key={index}>
                            <td>{index + 1}</td>
                            <td>{dataa.student_name}</td>
                            <td>
                              <input
                                type="text"
                                className="form-control"
                                name="total_marks"
                                value={dataa.total_marks}
                                onChange={(e) => {
                              ...
                              <div className="siderbar-toggle">
                                ...
                                <label className="switch">
                                  <input
                                    ...
                                    checked={dataa.multiplied_by_s === "yes"}
                                    onChange={(e) =>
                                      handleInputChange(e, dataa.id)
                                    }
                                  />
                              ...
                              <div className="siderbar-toggle">
                                ...
                                <label className="switch">
                                  <input
                                    ...
                                    checked={dataa.multiplied_by_p === "yes"}
                                    onChange={(e) =>
                                      handleInputChange(e, dataa.id)
                                    }
                                  />
                              ...
                              <div className="siderbar-toggle">
                                ...
                                <label className="switch">
                                  <input
                                    type="checkbox"
                                    name="divided_by_s"
                                    checked={dataa.divided_by_s === "yes"}
                                    onChange={(e) =>
                                      handleInputChange(e, dataa.id)
                                    }
                                  />
                              ...
                              <div className="siderbar-toggle">
                                ...
                                <label className="switch">
                                  <input
                                    type="checkbox"
                                    name="divided_by_p"
                                    checked={dataa.divided_by_p === "yes"}
                                    onChange={(e) =>
                                      handleInputChange(e, dataa.id)
                                    }
                                  />
                              ...
                              <div className="siderbar-toggle">
                                ...
                                <label className="switch">
                                  <input
                                    type="checkbox"
                                    name="simplification"
                                    checked={dataa.simplification === "yes"}
                                    onChange={(e) =>
                                      handleInputChange(e, dataa.id)
                                    }
                                  />
...
export default EditMathsReport;
