...
const FeesReports = () => {
  ...
  const filterDataBySearch = (data) => {
    return data.filter((item) =>
      getStudentById(data.id_student)
        .toLowerCase()
  ...
  const getStudentById = (bId) => {
    const foundStudent = student.find((b) => b.id === bId);
                        ...
                        currentItems.map((dataa, index) => {
                          return (
                            <tr key={index}>
                              ...
                              <td>{dataa.total_amount_paid + dataa.remaining_amount}</td>
                              <td>{dataa.total_amount_paid}</td>
                              <td style={{ color: '#e50012' }}>{dataa.remaining_amount}</td>
                            </tr>
                          );
                        })
                      )}
