...
const HomeworkStatusReport = () => {
  ...
  const getAssignHomeworkData = async () => {
    try {
      const res = await axios.get(
        `${process.env.REACT_APP_BACKEND_BASE_URL}/statusHomework`
      );
      if (res.status === 200) {
        if (userData.roll === "Admin") {
          ...
          <div className="content">
            <nav aria-label="breadcrumb" style={{ '--bs-breadcrumb-divider': 'none' }}>
              <ol className="breadcrumb">
                ...
                <li className="breadcrumb-item active" aria-current="page">Homework Status Report</li>
              </ol>
            </nav>
            <div className="content-page-header content-page-headersplit">
              <h5>Homework Status Report</h5>
            </div>
                    ...
                    <tbody>
                      {currentItems.length === 0 ? (
                        ...
                        currentItems.map((dataa, index) => {
                          const studentIdsString = dataa.not_done ?? "";
                          ...
                          const totalStudents =
                            (dataa.done_count ?? 0) + (dataa.not_done_count ?? 0);
                          const presentPercentage =
                            ((dataa.done_count ?? 0) / totalStudents) * 100;
                          ...
                          return (
                            <tr key={index}>
                              ...
                              <td>{dataa.done_count ?? 0}</td>
                              <td
                                data-bs-toggle="tooltip"
                                data-bs-placement="top"
                                title={studentNames}
                              >
                                {dataa.not_done_count ?? 0}
                              </td>
                              <td>{presentPercentage.toFixed(2) ?? 0}%</td>{" "}
                              <td>
                                {(100 - presentPercentage).toFixed(2) ?? 0}%
                              </td>{" "}
                            </tr>
                          );
                        })
