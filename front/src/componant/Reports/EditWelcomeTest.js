...
const EditWelcomeTest = () => {
  ...
  const getAddWelcomeData = async () => {
    try {
      const res = await axios.get(`${process.env.REACT_APP_BACKEND_BASE_URL}/getEditWelcomeTest/${id}`);
      if (res.status === 200) {
  ...
  const handleInputChange = (e, studentId) => {
    ...
    setData(prevValues => {
      const newData = prevValues.map(student => {
        if (student.id === studentId) { // Check against id
          return {
            ...student,
            [name]: checked ? "yes" : "no",
          };
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const res = await axios.put(
        `${process.env.REACT_APP_BACKEND_BASE_URL}/editWelcomeTest/${id}`,
        data
      );
  ...
  const handleDelete = async (id) => {
    try {
      const result = await Swal.fire({
        title: "Are you sure?",
        text: "You will be delete this English Report",
        icon: "warning",
      ...
      if (result.isConfirmed) {
        const res = await axios.delete(
          `${process.env.REACT_APP_BACKEND_BASE_URL}/deleteWelcomeTest/` + id
        );
        if (res.status === 200) {
          if (data.length === 1) {
            setData([]);
          ...
          Swal.fire({
            title: "Deleted!",
            text: "This Welcome Test has been deleted.",
            icon: "success",
          ...
          <div className="content">
            <nav aria-label="breadcrumb" style={{ '--bs-breadcrumb-divider': 'none' }}>
              <ol className="breadcrumb">
                ...
                <li className="breadcrumb-item"><NavLink to="/welcomereport">welcome Report </NavLink>/</li>
                <li className="breadcrumb-item active" aria-current="page">Edit Welcome Report</li>
              </ol>
            </nav>
            <div className="content-page-header content-page-headersplit">
              <h5>Edit Welcome Report</h5>
            </div>
            <div className="row">
                ...
                <div className="form-group">
                  ...
                  <div>
                    <select
                      ...
                      className="form-select"
                      disabled>
                      <option value="">{branch.board_name}</option>
                    </select>
                  </div>
                </div>
              </div>
                ...
                <div className="form-group">
                  ...
                  <div>
                    <select
                      ...
                      className="form-select"
                      disabled>
                      <option value="">{branch.medium_name}</option>
                    </select>
                  </div>
                </div>
              </div>
                ...
                <div className="form-group">
                  ...
                  <div>
                    <select
                      ...
                      className="form-select"
                      disabled>
                      <option value="">{branch.standard_name}</option>
                    </select>
                  </div>
                </div>
              </div>
                ...
                <div className="form-group">
                  ...
                  <div>
                    <select
                      ...
                      className="form-select"
                      disabled>
                      <option value="">{branch.batch_name}</option>
                    </select>
                  </div>
                </div>
              </div>
...
export default EditWelcomeTest