...
const ExamMarksReport = () => {
    ...
    const branchId = userData?.data.id_branch; const [currentPage, setCurrentPage] = useState(0); // Start from page 1
    ...
    const getStudentData = async () => {
        try {
            const res = await axios.get(`${process.env.REACT_APP_BACKEND_BASE_URL}/student`);
            if (res.status === 200) {
    ...
    const getStudentNameById = (subjectId) => {
        const foundStudent = studentAttendance.find((subject) => subject.id === subjectId);
        if (foundStudent) {
            const words = foundStudent.name.split(' ');
    ...
    const filterDataBySearch = (data) => {
        return data.filter((item) => {
            const studentName = getStudentNameById(item.id_student).toLowerCase();
            const examName = getExamNameById(item.id_exam).toLowerCase();
            const percentageStatus = calculatePercentage(item.marks, getExamTotalMarks(item.id_exam)).status.toLowerCase();
            const searchLower = searchQuery.toLowerCase();

            return studentName.includes(searchLower) ||
                examName.includes(searchLower) ||
                percentageStatus.includes(searchLower);
        });
    };
                    ...
                    <div className="content">
                        <nav aria-label="breadcrumb" style={{ '--bs-breadcrumb-divider': 'none' }}>
                            <ol className="breadcrumb">
                                ...
                                <li className="breadcrumb-item active" aria-current="page">Exam Result Report</li>
                            </ol>
                        </nav>
                        ...
                        <div className="row">
                            ...
                            <div className="col-lg-4">
                                <div className="form-group">
                                    <label>Batch</label>
                                    <div>
                                        <select name="id_batch" className="form-select" onChange={handleDropdownChange}>
                            ...
                            <div className="col-lg-4">
                                <div className="form-group">
                                    <label>Subject</label>
                                    <div>
                                        <select name="id_subject" className="form-select" onChange={handleDropdownChange}>
...
export default ExamMarksReport;
