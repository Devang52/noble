...
export default function EditGujaratiReport() {
  ...
  const getAddGujaratiData = async () => {
    try {
      const res = await axios.get(
        `${process.env.REACT_APP_BACKEND_BASE_URL}/getbyidgujaratireport/${id}`
      );
      if (res.status === 200) {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const res = await axios.put(
        `${process.env.REACT_APP_BACKEND_BASE_URL}/editgujaratireport/${id}`,
        data
      );
      ...
      if (res.status === 200) {
        ...
        Swal.fire({
          icon: "success",
          title: "Gujarati Report updated successfully",
          timer: 1500,
        }).then(() => {
        ...
        Swal.fire({
          title: "Gujarati Report update failed",
          icon: "error",
        });
      }
    } catch (err) {
  ...
  const handleDelete = async (id) => {
    try {
      const result = await Swal.fire({
        title: "Are you sure?",
        text: "You will be delete this Gujarati Report",
        icon: "warning",
      ...
      if (result.isConfirmed) {
        const res = await axios.delete(
          `${process.env.REACT_APP_BACKEND_BASE_URL}/deletegujaratireport/` + id
        );
        if (res.status === 200) {
          if (data.length === 1) {
            setData([]);
          ...
          Swal.fire({
            title: "Deleted!",
            text: "This Gujarati Report has been deleted.",
            icon: "success",
          ...
          Swal.fire({
            ...
            text: "Gujarati Report deleted failed.",
            icon: "error",
          });
        }
      }
          ...
          <div className="content">
            <nav aria-label="breadcrumb" style={{ '--bs-breadcrumb-divider': 'none' }}>
              <ol className="breadcrumb">
                ...
                <li className="breadcrumb-item active" aria-current="page">Edit Gujarati Report</li>
              </ol>
            </nav>
            <div className="content-page-header content-page-headersplit">
              <h5>Edit Gujarati Report</h5>
            </div>
            <div className="row">
                  ...
                  <table className="table datatable">
                    <thead>
                      <tr>
                        <th>Index</th>
                        <th>Student Name</th>
                        <th>મૂળાક્ષર ની ઓળખ</th>
                        <th>બારાક્ષરી</th>
                        <th>સાદા શબ્દો</th>
                        <th>કાના માત્રા વાળા શબ્દ</th>
                        <th>અર્ધ અક્ષર વાળા શબ્દ</th>
                        <th>અટકી અટકી ને વાચે છે</th>
                        <th>મોઢે લખાણ કેવુ છે</th>
                        <th>Hand Writing</th>
                        <th>Reading</th>
                      </tr>
                    </thead>
                            ...
                            <td>
                              <div className="siderbar-toggle">
                                ...
                                <label className="switch">
                                  <input
                                    type="checkbox"
                                    name="recognition_alphabets"
                                    checked={
                                      dataa.recognition_alphabets === "yes"
                                    }
                                    onChange={(e) =>
