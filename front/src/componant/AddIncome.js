  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const trimmedIncome = {
        ...
        amount: income.amount.trim(),
      };
      if (!trimmedIncome.title || !trimmedIncome.amount || !income.id_branch || !income.income_title || !income.income_type || !income.amount) {
        Swal.fire({
        ...
        const res = await axios.post(`${process.env.REACT_APP_BACKEND_BASE_URL}/income`, income);

        if (res.status === 200) {
          // Successful request
          Swal.fire({
            icon: 'success',
            title: 'Income Inserted Successfully',
            timer: 1500,
          }).then(() => {
            navigate('/income');
          });
