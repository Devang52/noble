    ...
    try {
      ...
      if (result.isConfirmed) {
        const res = await axios.delete(
          `${process.env.REACT_APP_BACKEND_BASE_URL}/attendance/delete/` + id
        );
        if (res.status === 200) {
          getAttendance();
          Swal.fire({
            title: "Deleted!",
            text: "This Attendance has been deleted.",
            icon: "success",
          ...
          Swal.fire({
            ...
            text: "Attendance deleted failed.",
            icon: "error",
          });
        }
      }
          ...
          <div className="content">
            ...
            <div className="content-page-header content-page-headersplit">
              ...
              <div className="list-btn">
                <ul>
                  <li>
                    <Link to={"/createattendance"} className="btn btn-primary">
                    ...
                    <tbody>
                      {currentItems.length === 0 ? (
                        ...
                        currentItems.map((dataa, index) => {
                          const totalStudents =
                            dataa.present_count + dataa.absent_count;
                          const presentPercentage =
                            (dataa.present_count / totalStudents) * 100;
                          // Check if the 'absent' property exists and is not null or undefined
                          const studentIdsString = dataa.absent || ""; // Use an empty string as fallback
                          ...
                          return (
                            <tr key={index}>
                              ...
                              <td>{(100 - presentPercentage).toFixed(2)}%</td>
                              {/* Show absent percentage */}
                              <td>
                                <div className="table-actions d-flex">
                                  <Link
                                    className="delete-table me-2"
                                    to={`/editattendance/${dataa.id}`}
                                  >
                                    {" "}
