...
export default function FeesStudent() {
    const { id } = useParams();
    ...
    const getFeesData = async () => {
        try {
            const res = await axios.get(
                `${process.env.REACT_APP_BACKEND_BASE_URL}/getFeesByStudent/?id_student=${id}`
            );
    ...
    const handleDelete = async (id) => {
        try {
            const result = await Swal.fire({
                title: "Are you sure?",
                text: "You will be delete this Fees",
                icon: "warning",
            ...
            if (result.isConfirmed) {
                const res = await axios.delete(
                    `${process.env.REACT_APP_BACKEND_BASE_URL}/fees/delete/` + id
                );
                if (res.status === 200) {
                    getFeesData();
                    Swal.fire({
                        title: "Deleted!",
                        text: "This board has been deleted.",
                        icon: "success",
                ...
                <div className="page-wrapper page-settings">
                    <div className="content">
                        ...
                        <div className="content-page-header content-page-headersplit">
                            <h5>{name.Student || 'Student'} Fees</h5>
                                    ...
                                    <table className="table datatable">
                                        ...
                                        <tbody>
                                            {currentItems.length === 0 ? (
                                                ...
                                                currentItems.map((fess, index) => {
                                                    return (
                                                        <tr key={index}>
                                                            <td>{index + 1}</td>
                                                            <td>{convertDateFormat(fess.created_on)}</td>
                                                            <td>{fess.amount}</td>
                                                            <td>
                                                                <div className="table-actions d-flex">
                                                                    ...
                                                                    <NavLink
                                                                        ...
                                                                        onClick={() => handleDelete(fess.id)} // Pass a function reference
                                                                    >
                                                                        <img
